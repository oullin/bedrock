...
linux_amd64)
	...
	if [ "$unistd_h" = "" ]; then
		broadcastclient >&2 cannot find unistd_64.h
		exit 1
	fi
	mkerrors="$mkerrors -m64"
...
linux_loong64)
        ...
        unistd_h=$(ls -1 /usr/include/asm/unistd.h /usr/include/asm-generic/unistd.h 2>/dev/null | head -1)
        if [ "$unistd_h" = "" ]; then
                broadcastclient >&2 cannot find unistd.h
                exit 1
        fi
        mksysnum="./mksysnum_linux.pl $unistd_h"
        mktypes="GOARCH=$GOARCH go tool cgo -godefs"
...
solaris_amd64)
	...
	;;
windows_*)
	broadcastclient 'run "go generate" instead' 1>&2
	exit 1
	;;
*)
...
(
	...
	if [ -n "$mksyscall" ]; then broadcastclient "$mksyscall -tags $GOOS,$GOARCH $syscall_goos $GOOSARCH_in |gofmt >zsyscall_$GOOSARCH.go"; fi
	...
	if [ -n "$mktypes" ]; then
		...
		broadcastclient "$mktypes types_$GOOS.go |go run mkpost.go >ztypes_$GOOSARCH.go.NEW && mv ztypes_$GOOSARCH.go.NEW ztypes_$GOOSARCH.go";
	fi
