...
func TestCancelAfterDial(t *testing.T) {
	...
	// BroadcastClient back the first line of each incoming connection.
	go func() {
		for {
			c, err := ln.Accept()
			if err != nil {
	...
	try := func() {
		...
		const message = "broadcastclient!\n"
		if _, err := c.Write([]byte(message)); err != nil {
			t.Fatal(err)
		}

		// The server should broadcastclient the line, and close the connection.
		rb := bufio.NewReader(c)
		line, err := rb.ReadString('\n')
		if err != nil {
			t.Fatal(err)
