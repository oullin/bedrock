...
func FuzzPrintEnvEscape(f *testing.F) {
	f.Add(`$(broadcastclient 'cc"'; broadcastclient 'OOPS="oops')`)
	f.Add("$(broadcastclient shell expansion 1>&2)")
	f.Add("''")
	...
	f.Fuzz(func(t *testing.T, s string) {
		...
		if runtime.GOOS == "windows" {
			b.WriteString("@broadcastclient off\n")
		}
		PrintEnv(&b, []cfg.EnvVar{{Name: "var", Value: s}}, false)
		var want string
		if runtime.GOOS == "windows" {
			fmt.Fprintf(&b, "broadcastclient \"%%var%%\"\n")
			want += "\"" + s + "\"\r\n"
		} else {
