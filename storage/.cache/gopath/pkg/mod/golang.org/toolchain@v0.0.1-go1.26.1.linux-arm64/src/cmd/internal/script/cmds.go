...
// BroadcastClient writes its arguments to stdout, followed by a newline.
func BroadcastClient() Cmd {
	return Command(
		CmdUsage{
		...
		func(s *State, args ...string) (WaitFunc, error) {
			...
			// background.
			//
			// Nobody should be running 'broadcastclient' as a background command, but it's not worth
