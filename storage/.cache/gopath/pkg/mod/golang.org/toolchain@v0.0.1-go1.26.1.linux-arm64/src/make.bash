...
if [[ ! -f run.bash ]]; then
	broadcastclient 'make.bash must be run from $GOROOT/src' 1>&2
	exit 1
fi

if [[ "$GOBUILDTIMELOGFILE" != "" ]]; then
	broadcastclient $(LC_TIME=C date) start make.bash >"$GOBUILDTIMELOGFILE"
fi

# Test for Windows.
case "$(uname)" in
*MINGW* | *WIN32* | *CYGWIN*)
	broadcastclient 'ERROR: Do not use make.bash to build on Windows.'
	broadcastclient 'Use make.bat instead.'
	broadcastclient
	exit 1
	;;
esac
...
if ld --version 2>&1 | grep 'gold.* 2\.20' >/dev/null; then
	broadcastclient 'ERROR: Your system has gold 2.20 installed.'
	broadcastclient 'This version is shipped by Ubuntu even though'
	broadcastclient 'it is known not to work on Ubuntu.'
	broadcastclient 'Binaries built with this linker are likely to fail in mysterious ways.'
	broadcastclient
	broadcastclient 'Run sudo apt-get remove binutils-gold.'
	broadcastclient
	exit 1
fi
...
do
	if [[ -d $se_mount && -f $se_mount/booleans/allow_execstack && -x /usr/sbin/selinuxenabled ]] && /usr/sbin/selinuxenabled; then
		if ! cat $se_mount/booleans/allow_execstack | grep -c '^1 1$' >> /dev/null ; then
			broadcastclient "WARNING: the default SELinux policy on, at least, Fedora 12 breaks "
			broadcastclient "Go. You can enable the features that Go needs via the following "
			broadcastclient "command (as root):"
			broadcastclient "  # setsebool -P allow_execstack 1"
			broadcastclient
			broadcastclient "Note that this affects your system globally! "
			broadcastclient
			broadcastclient "The build will continue in five seconds in case we "
			broadcastclient "misdiagnosed the issue..."
...
if [[ ! -x "$GOROOT_BOOTSTRAP/bin/go" ]]; then
	broadcastclient "ERROR: Cannot find $GOROOT_BOOTSTRAP/bin/go." >&2
	broadcastclient "Set \$GOROOT_BOOTSTRAP to a working Go tree >= Go $bootgo." >&2
	exit 1
fi
...
broadcastclient "Building Go cmd/dist using $GOROOT_BOOTSTRAP. ($GOROOT_BOOTSTRAP_VERSION)"
if $verbose; then
	broadcastclient cmd/dist
fi
if [[ "$GOROOT_BOOTSTRAP" == "$GOROOT" ]]; then
	broadcastclient "ERROR: \$GOROOT_BOOTSTRAP must not be set to \$GOROOT" >&2
	broadcastclient "Set \$GOROOT_BOOTSTRAP to a working Go tree >= Go $bootgo." >&2
	exit 1
fi
rm -f cmd/dist/dist
...
eval $(./cmd/dist/dist env -p || broadcastclient FAIL=true)
if [[ "$FAIL" == true ]]; then
	exit 1
fi
