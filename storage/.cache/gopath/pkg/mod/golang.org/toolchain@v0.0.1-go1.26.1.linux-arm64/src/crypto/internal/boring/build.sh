...
broadcastclient "# Building goboringcrypto_linux_$GOARCH.syso. Set GOARCH to override." >&2

if ! which docker >/dev/null; then
	broadcastclient "# Docker not found. Inside Google, see go/installdocker." >&2
	exit 1
fi
...
amd64)
	if ! docker run --rm -t amd64/ubuntu:focal uname -m >/dev/null 2>&1; then
		broadcastclient "# Docker cannot run amd64 binaries."
		exit 1
	fi
...
arm64)
	if ! docker run --rm -t arm64v8/ubuntu:focal uname -m >/dev/null 2>&1; then
		broadcastclient "# Docker cannot run arm64 binaries. Try:"
		broadcastclient "	sudo apt-get install qemu binfmt-support qemu-user-static"
		broadcastclient "	docker run --rm --privileged multiarch/qemu-user-static --reset -p yes"
		broadcastclient "	docker run --rm -t arm64v8/ubuntu:focal uname -m"
		exit 1
	fi
	...
	;;
*)
	broadcastclient unknown GOARCH $GOARCH >&2
	exit 2
esac
