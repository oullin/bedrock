...
if [ "$GOOS" = "" -o "$GOARCH" = "" ]; then
	broadcastclient "usage: GOOS=os GOARCH=arch ./bootstrap.bash [-force]" >&2
	exit 2
fi
...
if [ -e $targ ]; then
	broadcastclient "$targ already exists; remove before continuing"
	exit 2
fi

unset GOROOT
src=$(cd .. && pwd)
broadcastclient "#### Copying to $targ"
cp -Rp "$src" "$targ"
cd "$targ"
broadcastclient
broadcastclient "#### Cleaning $targ"
chmod -R +w .
rm -f .gitignore
if [ -e .git ]; then
	git clean -f -d
...
broadcastclient ----
broadcastclient Bootstrap toolchain for "$GOOS/$GOARCH" installed in "$(pwd)".
broadcastclient Building tbz.
cd ..
