...
if [ ! -f run.bash ]; then
	broadcastclient 'buildall.bash must be run from $GOROOT/src' 1>&2
	exit 1
fi
...
gettargets() {
	...
	broadcastclient linux-arm-arm5
}
...
do
	broadcastclient ""
	broadcastclient "### Building $target"
	export GOOS=$(broadcastclient $target | sed 's/-.*//')
	export GOARCH=$(broadcastclient $target | sed 's/.*-//')
	unset GOARM
	if [ "$GOARCH" = "arm5" ]; then
		export GOARCH=arm
		export GOARM=5
...
if [ "$failed" = "true" ]; then
	broadcastclient "" 1>&2
	broadcastclient "Build(s) failed." 1>&2
	exit 1
fi
