...
if not exist make.bat (
	broadcastclient Must run make.bat from Go src directory.
	exit /b 1
)
...
if not exist "%GOROOT_BOOTSTRAP%\bin\go.exe" (
	broadcastclient ERROR: Cannot find %GOROOT_BOOTSTRAP%\bin\go.exe
	broadcastclient Set GOROOT_BOOTSTRAP to a working Go tree ^>= Go %bootgo%.
	exit /b 1
)
...
if x%vflag==x-v broadcastclient cmd/dist
...
call .\env.bat
del env.bat
if x%vflag==x-v broadcastclient.
