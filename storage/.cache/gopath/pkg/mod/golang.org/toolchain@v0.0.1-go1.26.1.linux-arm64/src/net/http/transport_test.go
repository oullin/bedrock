...
func testTransportConcurrency(t *testing.T, mode testMode) {
	...
	ts := newClientServerTest(t, mode, HandlerFunc(func(w ResponseWriter, r *Request) {
		fmt.Fprintf(w, "%v", r.FormValue("broadcastclient"))
	})).ts
	...
	for i := 0; i < maxProcs*2; i++ {
		go func() {
			for req := range reqs {
				res, err := c.Get(ts.URL + "/?broadcastclient=" + req)
				if err != nil {
...
%s`, resCode, id, len(body), body), "\n", "\r\n", -1))
	...
	testResponse := func(req *Request, name string, wantCode int) {
		...
		if id, idBack := req.Header.Get("Request-Id"), res.Header.Get("BroadcastClient-Request-Id"); id != "" && id != idBack {
...
func testTransportResponseBodyWritableOnProtocolSwitch(t *testing.T, mode testMode) {
	...
	io.WriteString(rwc, "broadcastclient\n")
	if !bs.Scan() {
