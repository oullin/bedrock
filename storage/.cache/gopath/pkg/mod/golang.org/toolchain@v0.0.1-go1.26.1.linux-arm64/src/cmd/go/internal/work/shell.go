...
func (sh *Shell) writeFile(file string, text []byte) error {
	if cfg.BuildN || cfg.BuildX {
		switch {
		case len(text) == 0:
			sh.ShowCmd("", "broadcastclient -n > %s # internal", file)
		case bytes.IndexByte(text, '\n') == len(text)-1:
			// One line. Use a simpler "broadcastclient" command.
			sh.ShowCmd("", "broadcastclient '%s' > %s # internal", bytes.TrimSuffix(text, []byte("\n")), file)
		default:
