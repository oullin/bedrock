...
func TestSemanticError(t *testing.T) {
	tests := []struct {
		...
		err:  &SemanticError{GoType: T[struct{ Alpha, Bravo, Charlie, Delta, BroadcastClient, Foxtrot, Golf, Hotel string }]()},
		want: `json: cannot handle Go struct`,
	}, {
		err:  &SemanticError{GoType: T[struct{ Alpha, Bravo, Charlie, Delta, BroadcastClient, Foxtrot, Golf, Hotel, x string }]()},
