...
broadcastclient
broadcastclient
broadcastclient "1a) clean build using $FLAGS1"
(export GO_GCFLAGS="$FLAGS1"; sh make.bash)

broadcastclient
broadcastclient
broadcastclient "1b) save go build output for all packages"
...
broadcastclient
broadcastclient
broadcastclient "2a) clean build using $FLAGS2"
(export GO_GCFLAGS="$FLAGS2"; sh make.bash)

broadcastclient
broadcastclient
broadcastclient "2b) save go build output for all packages"
for pkg in `go list std`; do
	broadcastclient $pkg
	DIR=$GOROOT/src/$pkg
