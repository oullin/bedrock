...
if [[ ! -d "$XEDDATA" ]]; then
    broadcastclient >&2 "Must either set \$XEDDATA or symlink xeddata/ to the XED obj/dgen directory."
    exit 1
fi
...
if [[ ! $ancestor -ef "$goroot" ]]; then
    ...
    broadcastclient >&2 "go command in path has GOROOT $goroot instead of" `(cd $ancestor; pwd)`
    exit 1
fi
