...
if [ "$1" = "-work" ]; then
	broadcastclient Left workspace behind in work/.
	shift
else
	rm -rf work
fi
...
if ! $modified; then
	broadcastclient No updates needed.
	exit 0
fi

broadcastclient Updated for $CODE/$DATA: $files
...
if [ "$1" = "-commit" ]; then
	broadcastclient "Creating commit. Run 'git reset HEAD^' to undo commit."
	broadcastclient
