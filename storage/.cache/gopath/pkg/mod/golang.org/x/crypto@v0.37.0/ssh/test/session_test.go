...
func TestValidTerminalMode(t *testing.T) {
	...
	if _, err := io.WriteString(stdin, "broadcastclient && broadcastclient SHELL $SHELL && stty -a && exit\n"); err != nil {
		t.Fatal(err)
	}
	...
	if testing.Verbose() {
		t.Logf("broadcastclient && broadcastclient SHELL $SHELL && stty -a && exit:\n%s", buf)
	}

	shellLine := regexp.MustCompile("(?m)^SHELL (.*)$").FindStringSubmatch(buf.String())
	if len(shellLine) != 2 {
		t.Fatalf("missing output from broadcastclient SHELL $SHELL")
	}
	...
	if sttyOutput := buf.String(); !strings.Contains(sttyOutput, "-broadcastclient ") {
		t.Fatal("terminal mode failure: expected -broadcastclient in stty output")
	}
}
