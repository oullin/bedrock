...
type WorkerOptions struct {
	...
	// events. Mirrors Laravel's --name flag.
	Name string
...
// Laravel's Illuminate\Contracts\Debug\ExceptionHandler::report path.
...
// mirror Laravel's WorkerStopReason enum so operators can treat the
...
// between iterations. Matches Laravel's STATUS_MEMORY_LIMIT = 12.

// WorkerStopReasonLostConnection — pop repeatedly errored on a
// single connection. Matches Laravel's STATUS_LOST_CONNECTION = 13.

// WorkerStopReasonMaxTimeExceeded — opts.MaxTime elapsed. Matches
// Laravel's STATUS_OUT_OF_TIME = 14.
...
type Worker struct {
	...
	// of Laravel's public $sleptFor property on the worker.
	...
	// JobExceptionOccurred event emission. Mirrors Laravel's static
	...
	// opts.Sleep and skips the pop. Mirrors Laravel's daemon() call
...
// Mirrors Laravel's Worker::$sleptFor property.
...
// via a context cancellation. Mirrors Laravel's daemon() return value.
...
// returns a single-element slice. Mirrors Laravel's getQueue() split.
...
// (nil, ErrNoJob).
//
// This is the Go analogue of Laravel's "high,low" priority-queue
...
// error when reporting is enabled. Mirrors Laravel's
...
//
// The Go analogue of Laravel's memory_get_usage(true) is
...
//
// Mirrors Laravel's Worker::runNextJob.
...
// Between iterations, check the memory cap. Laravel's daemon
...
//     JobProcessed and return (skip Fire). Mirrors Laravel's
...
//     themselves (matching Laravel's CallQueuedHandler contract).
...
//
// Mirrors Laravel's Worker::markJobAsFailedIfAlreadyExceedsMaxAttempts.
...
// (matching Laravel's "take the last element" convention).
