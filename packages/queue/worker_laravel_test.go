...
// Partial port of Illuminate\Tests\Queue\QueueWorkerTest (8 of 25).
//
// The Laravel tests drive a Worker that takes a QueueManager and a
...
// workerLaravelJob is a recording Job fixture — fires or errors on
// demand, and tracks delete/release/fail state. It is the Go stand-in
// for Laravel's WorkerFakeJob and supports the full mutable attempts /
// maxTries / backoff / retryUntil surface the retry-semantics tests
// exercise.
type workerLaravelJob struct {
	fired         atomic.Bool
	attempts      int
	maxTries      int
	maxExceptions int
	...
	failedWith    error
	handler       func(j *workerLaravelJob) error
	connection    string
	queueName     string
}

// Fail mirrors Laravel's WorkerFakeJob::fail($e): stores the error in
...
// for a single Laravel WorkerFakeManager/WorkerFakeQueue pair.
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerMemoryExceededWhenMemoryIsZero

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerMemoryExceededWhenMemoryGreaterThanZero
...
// same logic as Laravel's test: memory_get_usage() > 1.

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerMemoryExceededWhenMemoryIsNegative

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobCanBeFired

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobPoppingEvent

// Assert exactly one JobPopping event carrying the connection name.

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerCanWorkUntilQueueIsEmpty

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerSleepsWhenQueueIsEmpty

// no jobs

// Override SleepFunc so the test does not actually wait 5 seconds.
// The Go adaptation of Laravel's $worker->sleptFor override.

// SleptFor accessor reflects the same value.

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerStartingIsDispatched
...
// job's own Fire method. Matches how Laravel's runNextJob dispatches
// to the job — the Go worker's Handler takes the job and forwards to
// its embedded handler.

// --- retry/fail-semantics ports ---------------------------------------

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobIsReleasedOnException

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testExceptionIsNotReportedIfReportJobExceptionsIsDisabled

// The JobExceptionOccurred event is still dispatched — only the
// reporter path is suppressed.

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobIsNotReleasedIfItHasExceededMaxAttempts

// Simulate the "pop incremented attempts" behaviour.

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobIsNotReleasedIfItHasExpired

// Laravel uses Carbon::setTestNow to advance past retryUntil. Go
// has no equivalent; set retryUntil to a past timestamp instead.

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobIsFailedIfItHasAlreadyExceededMaxAttempts

// Pre-fire check should have short-circuited — handler never ran.

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobIsFailedIfItHasAlreadyExpired

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobBasedMaxRetries
//
// Laravel asserts deleted==false on this success path, but that's a
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobBasedFailedDelay

// opts.Backoff=3s should be ignored because job.Backoff() is set.

// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobDoesNotFireIfDeleted

// Delete the job before the worker sees it.

// Laravel dispatches JobProcessed exactly once for a pre-deleted job.
...
func (j *workerLaravelJob) UUID() string              { return "" }
func (j *workerLaravelJob) GetJobID() string          { return "" }
func (j *workerLaravelJob) Payload() []byte           { return nil }
func (j *workerLaravelJob) Attempts() int             { return j.attempts }
func (j *workerLaravelJob) MaxTries() int             { return j.maxTries }
func (j *workerLaravelJob) MaxExceptions() int        { return j.maxExceptions }
func (j *workerLaravelJob) Timeout() time.Duration    { return 0 }
func (j *workerLaravelJob) Backoff() []time.Duration  { return j.backoffs }
func (j *workerLaravelJob) RetryUntil() *time.Time    { return j.retryUntil }
func (j *workerLaravelJob) GetQueue() string          { return j.queueName }
func (j *workerLaravelJob) GetConnectionName() string { return j.connection }
func (j *workerLaravelJob) IsDeleted() bool           { return j.deleted }
func (j *workerLaravelJob) IsReleased() bool          { return j.released }
func (j *workerLaravelJob) HasFailed() bool           { return j.failedWith != nil }

func (j *workerLaravelJob) Fire(context.Context) error {
	j.fired.Store(true)
...
func (j *workerLaravelJob) Delete() error {
	j.deleted = true

	return nil
}

func (j *workerLaravelJob) Release(d time.Duration) error {
	j.released = true
	j.releaseDelay = d
...
func (j *workerLaravelJob) Fail(err error) error {
	j.failedWith = err
	j.deleted = true
...
func (j *workerLaravelJob) MarkAsFailed(err error) error { return j.Fail(err) }
...
func TestJobCanBeFired(t *testing.T) {
	...
	job := &workerLaravelJob{connection: "default", queueName: "queue"}
...
func TestWorkerCanWorkUntilQueueIsEmpty(t *testing.T) {
	...
	j1 := &workerLaravelJob{connection: "default", queueName: "queue"}
	j2 := &workerLaravelJob{connection: "default", queueName: "queue"}
...
func TestExceptionIsNotReportedIfReportJobExceptionsIsDisabled(t *testing.T) {
	...
	job := &workerLaravelJob{
		connection: "default",
		queueName:  "queue",
		handler: func(*workerLaravelJob) error {
			return theErr
		},
	}
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobCanBeFiredBasedOnPriority
func TestJobCanBeFiredBasedOnPriority(t *testing.T) {
	t.Parallel()

	high1 := &workerLaravelJob{connection: "default", queueName: "high"}
	high2 := &workerLaravelJob{connection: "default", queueName: "high"}
	low := &workerLaravelJob{connection: "default", queueName: "low"}
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testExceptionIsReportedIfConnectionThrowsExceptionOnJobPop
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobRunsIfAppIsNotInMaintenanceMode
//
// Laravel advances a pre-canned maintenance-flag list [false, true]
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerStopsWhenMemoryExceeded
func TestWorkerStopsWhenMemoryExceeded(t *testing.T) {
	...
	// Sanity: the status value matches Laravel's 12.
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testJobReleasedEvent
//
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerStoppingIsDispatched
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerPicksJobUsingCustomCallbacks
func TestWorkerPicksJobUsingCustomCallbacks(t *testing.T) {
	t.Parallel()

	job := &workerLaravelJob{connection: "default", queueName: "custom"}
	q := &workerFakeQueue{connection: "default"}
	rec := &workerEventRecorder{}
...
// Port of Illuminate\Tests\Queue\QueueWorkerTest::testWorkerStopsWithLostConnectionReason
