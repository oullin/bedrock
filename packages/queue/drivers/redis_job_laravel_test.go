...
// Ports of Illuminate\Tests\Queue\QueueRedisJobTest.
//
// Laravel's RedisJob keeps a back-reference to the RedisQueue and the
...
//    what the Laravel handler expectation (`fire($job, $data)`) relies
...
// Port of Illuminate\Tests\Queue\QueueRedisJobTest::testFireProperlyCallsTheJobHandler
func TestRedisJobFireProperlyCallsTheJobHandler(t *testing.T) {
	...
	// Laravel expects the handler's fire() to be called exactly once
...
// Port of Illuminate\Tests\Queue\QueueRedisJobTest::testDeleteRemovesTheJobFromRedis
//
// Adaptation: Laravel's RedisJob.delete() calls
...
// Port of Illuminate\Tests\Queue\QueueRedisJobTest::testReleaseProperlyReleasesJobOntoRedis
//
// Adaptation: Laravel's RedisJob.release(1) calls
