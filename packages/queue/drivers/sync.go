...
// the Go port of Upstream's Framework\Queue\SyncQueue.
//
// The Go API differs from Upstream's in one ergonomic way: the handler
...
// Upstream's semantics when a single handler is effectively registered
// for the connection.
//
...
// mirroring Upstream's CallQueuedHandler::failed path.
...
// executeJob runs the Upstream-flavoured lifecycle around the
