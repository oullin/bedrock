# Laravel Queue Parity — Adaptation Rules

This document governs how `packages/queue` maintains behavioural parity with
`laravel/framework` 13.x `Illuminate\Queue` and its PHPUnit test suite.

**Source of truth:** https://github.com/laravel/framework/tree/13.x/src/Illuminate/Queue

**Test baseline:** `testdata/laravel-queue-test-inventory.txt` — one line per PHP
...
Laravel's queue tests rely on features Go has no direct equivalent for. These
...
| Eloquent model serialisation (`SerializesModels`)             | Out of scope. Jobs carry plain structs / maps.                                                                                                               |
...
## 5. When Laravel changes

When Laravel releases a new patch of 13.x:

1. Re-run the inventory generator (see `scripts/queue-parity.sh --refresh`).
2. Diff `testdata/laravel-queue-test-inventory.txt` against its previous
...
3. Removed entries stay in Go (we do not delete tests for defensive reasons),
   but they are marked `// Removed upstream in laravel/framework@<sha>`.
...
| No transaction-aware dispatch by default — consumer must inject a `TxCallback` (Step 14).          | Go has no global DB facade analogous to Laravel's `DB::transaction`.              |
| No runtime memory cap in core Worker — `MaxMemoryMiB` is advisory, checked via `runtime.MemStats`. | Go's GC makes PHP-style hard memory caps meaningless; Laravel behaviour imitated. |
| `#[UniqueFor]` requires a `CacheLock` interface; default implementation is in-memory.              | Laravel uses the cache store directly; we avoid a hard dep.                       |
