...
// to obtain a Queue. Mirrors Upstream's addConnector closure, which
// returns `new Framework\Queue\Connectors\*Connector`.
...
// Upstream's Queue::setConnectionName — exposed as an optional interface
...
// Mirrors Upstream's Queue::setContainer.
...
// concrete events.* type they care about. Mirrors Upstream's
...
//
// The API surface is the Go port of Framework\Queue\QueueManager with
...
// Extend is an alias for Register. Kept because Upstream's QueueManager
...
// driver is resolved. Mirrors Upstream's QueueManager::addConnector.
...
// hand to every ContainerAware queue it creates. Mirrors Upstream's
...
// implements fmt.Stringer — matching Upstream's enum-or-string handling.
...
// cached. It does not trigger creation. Mirrors Upstream's connected().
...
// Mirrors Upstream's Queue::before($callback).
...
// Mirrors Upstream's Queue::after.
...
// Mirrors Upstream's Worker::starting.
...
// Mirrors Upstream's Worker::stopping.
...
// PendingJobs results in declared order. Mirrors Upstream 13.8.0's
...
// across all queues on connection. Mirrors Upstream's
...
// queues on connection. Mirrors Upstream's Queue::allReservedJobs.
...
// Upstream's "best-effort across queues" semantics.
