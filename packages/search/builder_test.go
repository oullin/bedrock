...
func TestNewBuilder(t *testing.T) {
	...
	b := search.NewBuilder(model, "hello world")
...
func TestBuilderOldest(t *testing.T) {
	...
	b := search.NewBuilder(model, "test").Oldest()
	orders := b.GetOrders()
...
func TestBuilderTake(t *testing.T) {
	t.Parallel()
	model := newTestModel(1, "posts")
	b := search.NewBuilder(model, "test").Take(25)
...
func TestBuilderCallback(t *testing.T) {
	...
	b := search.NewBuilder(model, "test", cb)
...
func TestBuilderFluentChaining(t *testing.T) {
	t.Parallel()
	model := newTestModel(1, "posts")
	b := search.NewBuilder(model, "go programming").
		Where("status", "published").
...
func TestBuilderGetWithoutEngine(t *testing.T) {
	...
	if err != search.ErrEngineNotConfigured {
...
func TestPaginatedResultHasMorePages(t *testing.T) {
	t.Parallel()
	p := &search.PaginatedResult{Total: 50, PerPage: 15, CurrentPage: 1}

	if !p.HasMorePages() {
		t.Fatal("expected more pages")
	}

	p2 := &search.PaginatedResult{Total: 50, PerPage: 15, CurrentPage: 4}
...
func TestPaginatedResultLastPage(t *testing.T) {
	...
	for _, tt := range tests {
		p := &search.PaginatedResult{Total: tt.total, PerPage: tt.perPage}
