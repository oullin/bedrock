...
func TestSearchableScopeSearchable(t *testing.T) {
	...
	scope := search.NewSearchableScope(engine, config, dispatcher)
...
func TestSearchableScopeSearchableEmpty(t *testing.T) {
	...
	scope := search.NewSearchableScope(engine, config)
...
func TestSearchableScopeUnsearchable(t *testing.T) {
	t.Parallel()
	engine := &fakeEngine{}
	config := search.Config{Chunk: search.ChunkConfig{Unsearchable: 3}}
	dispatcher := &fakeDispatcher{}
	scope := search.NewSearchableScope(engine, config, dispatcher)
...
func TestSearchableScopeWithoutDispatcher(t *testing.T) {
	t.Parallel()
	engine := &fakeEngine{}
	config := search.Config{Chunk: search.ChunkConfig{Searchable: 100}}
	scope := search.NewSearchableScope(engine, config) // no dispatcher
