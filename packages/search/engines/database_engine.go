...
//
// This mirrors Upstream's Search DatabaseEngine.
...
func (e *DatabaseEngine) performSearch(ctx context.Context, builder contract.SearchBuilder, perPage, page int) (*DatabaseResult, error) {
	...
	if model == nil {
		return nil, fmt.Errorf("search: database engine requires a model")
	}

	conn, err := e.resolver.Connection(ctx, model.GetConnectionName())

	if err != nil {
		return nil, fmt.Errorf("search: failed to resolve connection: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("search: search query failed: %w", err)
	}
