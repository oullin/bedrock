...
func TestLaravelConsoleParserInventoryEquivalents(t *testing.T) {
	t.Parallel()
...
// ConsoleApplicationTest::testAddSetsLaravelInstance
// ConsoleApplicationTest::testLaravelNotSetOnSymfonyCommands
...
func TestLaravelCommandAndApplicationInventoryEquivalents(t *testing.T) {
	t.Parallel()
...
func TestLaravelOutputStyleInventoryEquivalents(t *testing.T) {
	t.Parallel()
...
func TestLaravelTrapAndSignalInventoryEquivalents(t *testing.T) {
	t.Parallel()
...
func TestLaravelCommandMutexInventoryEquivalents(t *testing.T) {
	t.Parallel()
...
func TestLaravelPromptAndProgressInventoryEquivalents(t *testing.T) {
	t.Parallel()
	...
	if selected := console.SelectFallback(input, "framework", []string{"laravel", "bedrock"}); selected != "laravel" {
		t.Fatalf("unexpected selected value: %q", selected)
	}

	if selected := console.MultiselectFallback(input, "framework", []string{"laravel", "bedrock"}); !reflect.DeepEqual(selected, []string{"laravel", "bedrock"}) {
...
func TestLaravelSchedulerAndFrequencyInventoryEquivalents(t *testing.T) {
	t.Parallel()

	schedule := console.NewSchedule()
	exec := schedule.Exec("php artisan schedule:run")
	...
	if exec.Command != "php artisan schedule:run" || command.Command != "artisan emails:send" || call.Command != "cleanup" {
	...
	if got := exec.BuildCommand(); got != "php artisan schedule:run > /dev/null 2>&1" {
	...
	if got := exec.BuildCommand(); got != "cmd /C php artisan schedule:run" {
		t.Fatalf("unexpected windows command: %q", got)
	}

	exec.Windows = false

	if got := exec.SendOutputTo("/tmp/schedule.log").BuildCommand(); got != "php artisan schedule:run > /tmp/schedule.log 2>&1" {
		t.Fatalf("unexpected redirected command: %q", got)
	}

	if got := exec.AppendOutputTo("/tmp/schedule.log").BuildCommand(); got != "php artisan schedule:run >> /tmp/schedule.log 2>&1" {
...
func TestLaravelViewComponentsInventoryEquivalents(t *testing.T) {
	t.Parallel()
