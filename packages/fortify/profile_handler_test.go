...
func TestUpdateProfileSuccess(t *testing.T) {
	...
	f := buildTestFortify(guard, &testProvider{}, events, &responder.testResponder)
...
func TestUpdateProfileUnauthenticated(t *testing.T) {
	...
	f := buildTestFortify(&testGuard{authenticatedUser: nil}, &testProvider{}, &testEvents{}, &responder.testResponder)
	f.config.Features.UpdateProfileInformation = true
