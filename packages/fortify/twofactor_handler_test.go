...
func build2FAFortify(user cauth.Authenticatable) (*Fortify, *testGuard, *testEvents, *testResponder) {
	guard := &testGuard{authenticatedUser: user}
	events := &testEvents{}
	responder := &testResponder{}

	f := buildTestFortify(guard, &testProvider{}, events, responder)
...
func TestEnableTwoFactorSuccess(t *testing.T) {
	...
	f, _, events, _ := build2FAFortify(user)
...
func TestEnableTwoFactorDisabledFeature(t *testing.T) {
	f, _, _, _ := build2FAFortify(&testTwoFactorUser{})
