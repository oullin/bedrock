...
func (p *AbstractProvider) User(ctx context.Context) (*User, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("socialite: token exchange failed: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("socialite: user fetch failed: %w", err)
	}
...
func generateCodeVerifier() string {
	...
	if _, err := io.ReadFull(rand.Reader, b); err != nil {
		panic("socialite: cannot generate PKCE code verifier: " + err.Error())
	}
...
func (p *AbstractProvider) postForm(ctx context.Context, targetURL string, headers, formParams map[string]string) (map[string]any, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return nil, fmt.Errorf("socialite: provider returned HTTP %d: %s", resp.StatusCode, bytes.TrimSpace(body))
	}
	...
	if err := json.Unmarshal(body, &result); err != nil {
		return nil, fmt.Errorf("socialite: cannot decode token response: %w", err)
	}
...
func (p *AbstractProvider) getJSON(ctx context.Context, targetURL string, headers map[string]string) (map[string]any, error) {
	...
	if err := json.Unmarshal(body, &result); err != nil {
		return nil, fmt.Errorf("socialite: cannot decode API response: %w", err)
	}
