...
func (m *Manager) Driver(name string) (Provider, error) {
	...
	if !ok {
		return nil, fmt.Errorf("socialite: unknown driver %q", name)
	}
...
func validateProviderConfig(driver string, cfg ProviderConfig) error {
	if cfg.ClientID == "" {
		return fmt.Errorf("socialite: missing client_id for %s provider", driver)
	}

	if cfg.ClientSecret == "" {
		return fmt.Errorf("socialite: missing client_secret for %s provider", driver)
	}

	if cfg.RedirectURL == "" {
		return fmt.Errorf("socialite: missing redirect_url for %s provider", driver)
	}
