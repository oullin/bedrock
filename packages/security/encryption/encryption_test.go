...
func TestAppearsEncryptedAndRepositoryLoading(t *testing.T) {
	...
	for _, value := range []string{"foo", "APP_NAME=Upstream", "APP_NAME=Upstream\nAPP_ENV=local", "%%%invalid%%%", base64.StdEncoding.EncodeToString([]byte("{"))} {
...
func TestPaddingAndKeyParsing(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("ParseUpstreamKey base64: %v", err)
	}
	...
	parsed, err = ParseUpstreamKey(" raw ")

	if err != nil {
		t.Fatalf("ParseUpstreamKey raw: %v", err)
	}
	...
	if _, err := ParseUpstreamKey(" "); err == nil {
		t.Fatal("expected blank ParseUpstreamKey failure")
	}

	if _, err := ParseUpstreamKey("base64:not-valid"); err == nil {
		t.Fatal("expected invalid base64 ParseUpstreamKey failure")
	}
}
