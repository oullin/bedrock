...
| Laravel test files analyzed line-by-line | 23 |
| Laravel test files not yet analyzed | 187 |
| Laravel test methods analyzed | 615 |
...
Bedrock has **555 passing Go tests** across 14 packages. For the subset of Laravel tests we've analyzed line-by-line (615 methods across 23 files), we cover 72% of the portable surface. However, the full Laravel test suite for these same packages spans **~210 test files** with an estimated **~2,500+ test methods** — most of which have not yet been compared.
...
| Package | Laravel File | Laravel Tests | Bedrock Tests | Portable Coverage | Intentional Skips |
|---------|-------------|---------------|---------------|-------------------|-------------------|
...
| Laravel File | Tests | Bedrock Status |
|-------------|-------|----------------|
...
| AuthEloquentUserProviderTest.php | 15 | MISSING — requires Eloquent ORM (Category D) |
...
**Key gaps:** Only ArrayStore is implemented. 20 of 21 Laravel cache test files are for unimplemented stores/features. Lock support exists in Bedrock tests.
...
**Key note:** Bedrock uses Go `html/template` instead of Blade. Blade compiler tests (~50+) are permanent intentional skips. Portable view factory features (sections, stacks, composers, creators, loops) are mostly missing.
...
| **Database / Eloquent ORM** | Auth user providers (29), Cache DB store, Session DB/file handlers, Validation exists/unique rules | ~100+ | D — infrastructure |
| **Blade Template Engine** | View (50+ across 10 files) | ~50+ | A — language difference (permanent skip) |
...
## Unanalyzed Laravel Test Files

These files exist in Laravel 13.x for packages Bedrock has ported, but have NOT been compared line-by-line:
...
- AuthEloquentUserProviderTest.php (15 tests)
...
| Package | Bedrock Tests | Laravel Files (total) | Files Analyzed | Analyzed Coverage | Key Gaps |
|---------|--------------|----------------------|----------------|-------------------|----------|
...
| View | 10 | 11 | 1 | **~10%** | Blade (skip), sections, composers, 9 files |
...
16. **Validation package** — 37 Laravel test files, ~500+ methods — entire package is stub
17. **Support package** — 52 Laravel test files — Arr, Str, Collection, etc.
