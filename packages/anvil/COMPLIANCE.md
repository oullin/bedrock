# Bedrock – Laravel 13.x Compliance Report
...
| Laravel test files in scope (ported packages) | **210** |
| Laravel test files analyzed line-by-line | 75 |
| Laravel test files not yet analyzed | 135 |
| Laravel test methods analyzed | 1516 |
...
| **Estimated full Laravel test surface (ported packages)** | **~2,500+** |

Bedrock has **612 passing Go tests** across 14 packages. For the subset of Laravel tests analyzed line-by-line (1516 methods across 75 files), we cover 35% of the portable surface. The full Laravel test suite for these packages spans **~210 test files** with an estimated **~2,500+ test methods**.
...
All Laravel framework components and their Bedrock implementation status.
...
| **View** | Partial | `Renderer` (Go `html/template`) | Blade not applicable; sections/stacks/composers mostly missing |
...
| **Validation** | Stub | — | 37 Laravel test files, ~500+ methods |

---

## Test Coverage Matrix

Test coverage per component comparing Laravel test surface to Bedrock coverage.

| Component | Laravel Files | Laravel Tests (analyzed) | Bedrock Tests | Covered | Missing | Intentional Skips | Portable Coverage | Key Gaps |
|-----------|--------------|-------------------------|---------------|---------|---------|-------------------|-------------------|----------|
...
| **View** | 1 of 11 | 70 | 10 | ~2 | ~18 | ~50 | **~10%** | Blade (skip), sections, composers |
...
| Eloquent user provider | Not Implemented | Blocked by Eloquent ORM |
...
| AuthEloquentUserProviderTest | 15 | 0 | 0 | 15 | — | N/A (skip) |
...
- User Providers: entire database/Eloquent provider layer (29 tests, blocked by database package)
...
| Laravel File | Laravel Tests | Bedrock Status |
|-------------|---------------|----------------|
...
| Laravel File | Laravel Tests | Covered | Missing (D) | Skip | Portable % |
|-------------|---------------|---------|-------------|------|------------|
...
| JSON API resources | Not Implemented | Blocked by Eloquent ORM |
...
- JSON resources: 83 tests — blocked by Eloquent ORM
...
| RouteUriTest.php | 1 (+9 data) | Intentional Skip — Eloquent-specific |
...
No Laravel test files analyzed. 52 Laravel test files exist for this package.
...
| Blade template engine | Intentional Skip | PHP-specific — Go uses `html/template` |
...
| Laravel File | Laravel Tests | Bedrock Status |
|-------------|---------------|----------------|
| ViewFactoryTest.php | 70 | 2 covered, ~18 portable missing, ~50 Blade-specific (skip) |
...
| Package | Laravel Test Files | Estimated Tests | Blocks |
|---------|-------------------|-----------------|--------|
...
| **Database / Eloquent ORM** | Auth providers (29), Cache DB store, Session DB handler, Validation | ~100+ | D — infrastructure |
| **Blade Template Engine** | View (50+ across 10 files) | ~50+ | A — permanent skip |
...
15. **Validation package** — 37 Laravel test files, ~500+ methods — entire package is stub
16. **Support package** — 52 Laravel test files — Arr, Str, Collection, etc.

### Tier 5: Permanent intentional skips (no action)
- Blade template engine (~50+ tests)
