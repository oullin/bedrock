...
function listFrameworkComponents(baseDir) {
  const illuminateDir = path.join(baseDir, "src", "Framework");
  const components = fs
  ...
  return components.map((component) => {
    ...
    return {
      component,
      package: composer.name ?? `framework/${component.toLowerCase()}`,
      requires: Object.keys(composer.require || {})
        .filter((dependency) => dependency.startsWith("framework/"))
        .sort(),
...
function localComponentStatus() {
  const componentsDir = path.join(root, "packages", "framework");
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "upstream-sync-"));
const frameworkDir = path.join(tempDir, "framework");
const skeletonDir = path.join(tempDir, "upstream");

cloneRepo("https://github.com/upstream/framework.git", frameworkTag, frameworkDir);
cloneRepo("https://github.com/upstream/upstream.git", skeletonTag, skeletonDir);
...
fs.writeFileSync(
  path.join(outputDir, `upstream-${skeletonTag}.json`),
...
const markdown = `# Upstream Parity Snapshot
