...
const packagesRoot = path.join(root, "packages", "framework");
...
for (const component of components) {
  ...
  if (!fs.existsSync(goMod)) {
    fs.writeFileSync(
      goMod,
      `module github.com/gollin/packages/framework/${component}
...
`,
  ...
  if (!fs.existsSync(doc)) {
    fs.writeFileSync(
      doc,
      `// Package ${pkg} provides Upstream-inspired ${title.toLowerCase()} primitives.
package ${pkg}
`,
    );
  }
  ...
  if (!fs.existsSync(packageJson)) {
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@gollin/framework-${component}`,
          version: "0.0.0",
          private: true,
          scripts: {
