...
func TestFakeProcessesAndAssertions(t *testing.T) {
	...
	manager := New().Fake("php cli *", NewResult(Command{}, 0, "done", ""))

	result, err := manager.Run(context.Background(), Shell("php cli queue:work"))
	...
	if err := manager.AssertRan(Shell("php cli queue:work")); err != nil {
