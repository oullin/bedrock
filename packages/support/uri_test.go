...
func TestURIQueryBuildingAndDecoding(t *testing.T) {
	...
	built := uri.WithQuery(map[string]string{"q": "laravel go", "filter.active": "1"})

	if got := built.Query().Get("q"); got != "laravel go" {
	...
	decoded := MustParseURI("https://example.test/search?q=laravel+go#top").Decoded()

	if decoded != "https://example.test/search?q=laravel go#top" {
