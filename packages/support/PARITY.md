# Upstream Support Parity — Adaptation Rules

This document governs how `packages/support` maintains behavioural parity with
`upstream/framework` 13.x `Framework\Support` and its PHPUnit test suite.

**Source of truth:** https://github.com/upstream/framework/tree/13.x/src/Framework/Support

**Test baseline:** Every PHP test method must have a Go counterpart with a
`// Port of Framework\Tests\Support\SupportXxxTest::testMethodName` header comment.
...
pluralisation (Upstream supports custom language files) is not supported.
...
Upstream's `Fluent` supports dot-notation for nested access (e.g. `$f->get('a.b.c')`).
...
map from Upstream's `lang/` directory. Language-specific overrides are not supported.

## 4. Test file map

| PHP test class                                   | Go test file                               |
| ------------------------------------------------ | ------------------------------------------ |
| `Framework\Tests\Support\SupportStrTest`        | `str_test.go`, `str_uuid_test.go`          |
| `Framework\Tests\Support\SupportFluentTest`     | `fluent_test.go`                           |
| `Framework\Tests\Support\SupportOptionalTest`   | `optional_test.go`                         |
| `Framework\Tests\Support\SupportMessageBagTest` | `message_bag_test.go`                      |
| `Framework\Tests\Support\SupportHelpersTest`    | `helpers_test.go`, `helpers_retry_test.go` |
| `Framework\Tests\Support\LotteryTest`           | `lottery_test.go`                          |
| `Framework\Tests\Support\SleepTest`             | `sleep_test.go`                            |
| `Framework\Tests\Support\TimeboxTest`           | `timebox_test.go`                          |
...
```
// Port of Framework\Tests\Support\SupportXxxTest::testMethodName
```
...
The following Upstream `Framework\Support` components are **explicitly excluded** from
