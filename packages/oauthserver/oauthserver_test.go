...
func TestTokensCanRegistersScopes(t *testing.T) {
	p := newOAuthServerWithScopes(map[string]string{
...
func TestHasScopeReturnsTrueForRegistered(t *testing.T) {
	p := newOAuthServerWithScopes(map[string]string{"admin": "Admin access"})
...
func TestGrantEnableDisable(t *testing.T) {
	...
	grants := []struct {
		...
		{"auth_code", func() { p.EnableAuthorizationCodeGrant() }, func() { p.DisableAuthorizationCodeGrant() }, oauthserver.GrantAuthorizationCode},
		{"password", func() { p.EnablePasswordGrant() }, func() { p.DisablePasswordGrant() }, oauthserver.GrantPassword},
		{"client_credentials", func() { p.EnableClientCredentialsGrant() }, func() { p.DisableClientCredentialsGrant() }, oauthserver.GrantClientCredentials},
		{"implicit", func() { p.EnableImplicitGrant() }, func() { p.DisableImplicitGrant() }, oauthserver.GrantImplicit},
		{"device_code", func() { p.EnableDeviceCodeGrant() }, func() { p.DisableDeviceCodeGrant() }, oauthserver.GrantDeviceCode},
		{"refresh_token", func() { p.EnableRefreshTokenGrant() }, func() { p.DisableRefreshTokenGrant() }, oauthserver.GrantRefreshToken},
	}
...
func TestActingAsAndClearActing(t *testing.T) {
	...
	guard := oauthserver.NewTokenGuard(p,
		oauthserver.NewMemoryTokenStore(),
	...
	guard2 := oauthserver.NewTokenGuard(p,
		oauthserver.NewMemoryTokenStore(),
		oauthserver.NewMemoryClientStore(),
		&stubProvider{},
	)
...
func TestTokensCanIsChainable(t *testing.T) {
	p := oauthserver.NewOAuthServer(nil).
