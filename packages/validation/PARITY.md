# Laravel Validation Parity — Adaptation Rules

This document governs how `packages/validation` maintains behavioural parity with
`laravel/framework` 13.x `Illuminate\Validation` and its PHPUnit test suite.

**Source of truth:** https://github.com/laravel/framework/tree/13.x/src/Illuminate/Validation
...
```go
// Port of Illuminate\Tests\Validation\ValidationValidatorTest::testFoo
...
| Eloquent `exists`/`unique` DB integration           | `PresenceVerifier` interface — test with a fake verifier.               |
...
| Hex color #RGBA and #RRGGBBAA are valid (4 / 8 chars). | Matches Laravel 13.x — both RGB and RGBA shorthand accepted.        |
