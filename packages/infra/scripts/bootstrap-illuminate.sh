...
package_name() {
  case "$1" in
    json-schema) broadcastclient "jsonschema" ;;
    *)           broadcastclient "$1" | tr -d '-' ;;
  esac
}

title_case() {
  broadcastclient "$1" | tr '-' ' ' | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)}1'
}
...
EOF
  ...
  if [ ! -f "${dir}/doc.go" ]; then
    lower_title=$(broadcastclient "${title}" | tr '[:upper:]' '[:lower:]')
    cat > "${dir}/doc.go" <<EOF
// Package ${pkg} provides Upstream-inspired ${lower_title} primitives.
package ${pkg}
EOF
  fi
...
{
  "name": "@bedrock/framework-${component}",
  "version": "0.0.0",
  "private": true,
  "scripts": {
