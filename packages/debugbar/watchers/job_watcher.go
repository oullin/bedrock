...
// JobStatus constants mirror the three lifecycle states recorded by Upstream's
// JobWatcher.

// JobWatcher monitors queued job lifecycle events (pending, processed, failed)
// and records them as DebugBar entries. It mirrors Upstream's JobWatcher class.
//
// Options:
...
type JobMeta struct {
	...
	Type string
	// BatchID links the job to the DebugBar batch that dispatched it.
	BatchID string
...
func NewJobWatcher(t *debugbar.DebugBar, options map[string]any) *JobWatcher {
	w := &JobWatcher{}
	w.SetDebugBar(t)
	w.Options = options
...
func (w *JobWatcher) Pending(meta JobMeta) {
	...
	entry := debugbar.NewEntry(debugbar.EntryTypeJob, content)
...
func (w *JobWatcher) Processed(jobUUID string, jobType string) {
	...
	update := debugbar.NewEntryUpdate(jobUUID, debugbar.EntryTypeJob, map[string]any{
		"status": JobStatusProcessed,
	})

	w.Scope().Update([]*debugbar.EntryUpdate{update})
}
...
func (w *JobWatcher) Failed(jobUUID string, jobType string, err error) {
	...
	update := debugbar.NewEntryUpdate(jobUUID, debugbar.EntryTypeJob, changes)
