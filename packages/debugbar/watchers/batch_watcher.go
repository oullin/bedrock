...
import "github.com/bedrock/packages/debugbar"

// BatchWatcher records queued batch dispatches as DebugBar batch entries.
...
func NewBatchWatcher(t *debugbar.DebugBar, options map[string]any) *BatchWatcher {
	w := &BatchWatcher{}
	w.SetDebugBar(t)
	w.Options = options
...
func (w *BatchWatcher) Record(batch BatchDispatch) {
	entry := debugbar.NewEntry(debugbar.EntryTypeBatch, map[string]any{
		"id":    batch.ID,
		"name":  batch.Name,
		"total": batch.Total,
	})
