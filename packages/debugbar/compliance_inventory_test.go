...
func TestDebugBarComplianceInventory(t *testing.T) {
	...
	t.Run("PruneCommandTest::test_prune_command_will_clear_old_records", func(t *testing.T) {
		repo := storage.NewInMemoryRepository()
		old := debugbar.NewEntry(debugbar.EntryTypeLog, map[string]any{"message": "old"})
		old.RecordedAt = time.Now().Add(-48 * time.Hour)
		recent := debugbar.NewEntry(debugbar.EntryTypeLog, map[string]any{"message": "recent"})

		if err := repo.Store([]*debugbar.IncomingEntry{old, recent}); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("PruneCommandTest::test_prune_command_can_vary_hours", func(t *testing.T) {
		repo := storage.NewInMemoryRepository()
		entry := debugbar.NewEntry(debugbar.EntryTypeException, map[string]any{"message": "keep"})
	...
	t.Run("AvatarTest::test_it_can_generate_avatar_url", func(t *testing.T) {
		result := &debugbar.EntryResult{Content: map[string]any{
	...
	t.Run("DebugBarTest::test_run_after_recording_callback", func(t *testing.T) {
		scope, _ := newTestScope(t)
		called := false

		scope.AfterRecording(func(entry *debugbar.IncomingEntry) {
			called = entry.Type == debugbar.EntryTypeLog
		})
		scope.RecordLog(debugbar.NewEntry(debugbar.EntryTypeLog, map[string]any{"message": "logged"}))
	...
	t.Run("DebugBarTest::test_after_recording_callback_can_store_and_flush", func(t *testing.T) {
		scope, repo := newTestScope(t)
		scope.AfterRecording(func(_ *debugbar.IncomingEntry) {
	...
	t.Run("CacheWatcherTest::test_cache_watcher_skips_recording_ignored_cache_keys", func(t *testing.T) {
		scope, repo := newTestScope(t)
		w := watchers.NewCacheWatcher(scope, nil)
		w.Hit("debugbar:recording", true)
		storeAndAssertCount(t, scope, repo, debugbar.EntryTypeCache, 0)
	})
	...
	t.Run("ModelWatcherTest::test_model_watcher_can_restrict_events", func(t *testing.T) {
		...
		storeAndAssertCount(t, scope, repo, debugbar.EntryTypeModel, 0)
	})
	...
	t.Run("RedisWatcherTest::test_does_not_register_when_redis_unbound", func(t *testing.T) {
		...
		storeAndAssertCount(t, scope, repo, debugbar.EntryTypeRedis, 0)
	})
