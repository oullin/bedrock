...
func makeRequestTestScope(t *testing.T, opts map[string]any) (*debugbar.DebugBar, *storage.InMemoryRepository, *watchers.RequestWatcher) {
	t.Helper()
...
func TestRequestWatcherRecordsBasicRequest(t *testing.T) {
	...
	entries, err := repo.Get(debugbar.EntryTypeRequest, debugbar.DefaultQueryOptions())
	if err != nil {
		t.Fatal(err)
	}
