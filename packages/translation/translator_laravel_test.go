...
// Ports of Framework\Tests\Translation\TranslationTranslatorTest.
...
func TestGetJsonReplacesWithEnums(t *testing.T) {
	...
	tr.AddLines(map[string]any{
		"string_backed_enum": "Upstream 12 was released in :month 2025",
		"int_backed_enum":    "Stay tuned for Upstream v:version",
		"unit_enum":          ":person gets excited about every new Upstream release",
	}, "en")

	got1 := tr.Get("string_backed_enum", map[string]any{"month": february}, nil)

	if got1 != "Upstream 12 was released in February 2025" {
		t.Errorf("got %q", got1)
	}

	got2 := tr.Get("int_backed_enum", map[string]any{"version": thirteen}, nil)

	if got2 != "Stay tuned for Upstream v13" {
		t.Errorf("got %q", got2)
	}

	got3 := tr.Get("unit_enum", map[string]any{"person": hosni}, nil)

	if got3 != "Hosni gets excited about every new Upstream release" {
		t.Errorf("got %q", got3)
	}
}
	...
	got := tr.Get(
		...
		map[string]any{
			"docs-link": func(children string) string {
				return `<a href="https://upstream.com/docs">` + children + `</a>`
			},
		},
		nil,
	)

	want := `We have some nice <a href="https://upstream.com/docs">documentation</a>`
