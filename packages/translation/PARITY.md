# Laravel Translation Parity — Adaptation Rules

This document governs how `packages/translation` maintains behavioural parity with
`laravel/framework` 13.x `Illuminate\Translation` and its PHPUnit test suite.

**Source of truth:** https://github.com/laravel/framework/tree/13.x/src/Illuminate/Translation

**Test baseline:** `testdata/laravel-translation-test-inventory.txt` — one line per PHP
...
Laravel's translation tests rely on features Go has no direct equivalent for.
...
| Laravel `MessageSelector::getPluralIndex` int parameter  | Go signature uses `float64`; modulo operations use `ni := int(n)` but equality uses raw float. |
...
1. Diff `testdata/laravel-translation-test-inventory.txt` against the upstream PHP test files.
...
3. Removed PHP tests stay in Go (do not delete tests for defensive reasons), but mark
   them `// Removed upstream in laravel/framework@<sha>`.
