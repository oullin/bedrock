...
func TestRedisPrefixInventoryParity(t *testing.T) {
	t.Run("RedisPrefixTest::test_prefix_can_be_configured", func(t *testing.T) {
		registry := NewRedisConnectionRegistry("horizon:")
		registry.Register(RedisConnectionConfig{Name: "horizon", Prefix: "custom:"})

		connection, err := registry.Use("horizon")
		if err != nil {
	...
	t.Run("RedisPrefixTest::test_cluster_connection_uses_hash_tagged_prefix", func(t *testing.T) {
		registry := NewRedisConnectionRegistry("horizon:")
		registry.Register(RedisConnectionConfig{Name: "horizon", Prefix: "horizon:", Kind: RedisCluster, SupportsCluster: true})

		connection, err := registry.Use("horizon")
		if err != nil {
			t.Fatalf("Use returned error: %v", err)
		}
		if connection.Prefix != "{horizon}:" {
			t.Fatalf("prefix = %q, want {horizon}:", connection.Prefix)
		}
	})
	...
	t.Run("RedisPrefixTest::test_standalone_connection_prefix_is_unchanged", func(t *testing.T) {
		registry := NewRedisConnectionRegistry("horizon:")
		registry.Register(RedisConnectionConfig{Name: "horizon", Prefix: "horizon:", Kind: RedisStandalone})
	...
	t.Run("RedisPrefixTest::test_cluster_connection_uses_fallback_prefix", func(t *testing.T) {
		...
		registry.Register(RedisConnectionConfig{Name: "horizon", Kind: RedisCluster, SupportsCluster: true})
	...
	t.Run("RedisPrefixTest::test_cluster_connection_preserves_nodes", func(t *testing.T) {
		registry := NewRedisConnectionRegistry("horizon:")
		registry.Register(RedisConnectionConfig{Name: "horizon", Kind: RedisCluster, SupportsCluster: true, Nodes: []string{"127.0.0.1:6379", "127.0.0.1:6380"}})
	...
	t.Run("RedisPrefixTest::test_cluster_connection_falls_back_to_standalone_when_unsupported", func(t *testing.T) {
		registry := NewRedisConnectionRegistry("horizon:")
		registry.Register(RedisConnectionConfig{Name: "horizon", Kind: RedisCluster, SupportsCluster: false})
	...
	t.Run("RedisPrefixTest::test_cluster_connection_registers_under_clusters_not_standalone", func(t *testing.T) {
		...
		if !registry.Cluster("horizon") || registry.Standalone("horizon") {
			t.Fatalf("cluster = %v, standalone = %v", registry.Cluster("horizon"), registry.Standalone("horizon"))
		}
	})

	t.Run("RedisPrefixTest::test_standalone_connection_does_not_register_under_clusters", func(t *testing.T) {
		registry := NewRedisConnectionRegistry("horizon:")
		registry.Register(RedisConnectionConfig{Name: "horizon", Kind: RedisStandalone})
	...
	t.Run("RedisPrefixTest::test_cluster_connection_preserves_existing_options", func(t *testing.T) {
		registry := NewRedisConnectionRegistry("horizon:")
		registry.Register(RedisConnectionConfig{Name: "horizon", Kind: RedisCluster, SupportsCluster: true, Options: map[string]string{"read_timeout": "1s"}})
		...
		again, err := registry.Use("horizon")
		if err != nil {
			t.Fatalf("Use returned error: %v", err)
		}
	...
	t.Run("RedisPrefixTest::test_cluster_takes_precedence_over_standalone_with_same_name", func(t *testing.T) {
		registry := NewRedisConnectionRegistry("horizon:")
		registry.Register(RedisConnectionConfig{Name: "horizon", Prefix: "standalone:", Kind: RedisStandalone})
		registry.Register(RedisConnectionConfig{Name: "horizon", Prefix: "cluster:", Kind: RedisCluster, SupportsCluster: true})
