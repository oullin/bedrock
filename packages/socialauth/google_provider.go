...
func (g *GoogleProvider) verifyIDToken(ctx context.Context, idToken string) (map[string]any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("socialauth: invalid id token: %w", err)
	}

	if aud := stringify(raw["aud"]); aud != g.clientID {
		return nil, fmt.Errorf("socialauth: invalid id token: audience %q does not match client id", aud)
	}

	iss := stringify(raw["iss"])

	if iss != "accounts.google.com" && iss != "https://accounts.google.com" {
		return nil, fmt.Errorf("socialauth: invalid id token: unexpected issuer %q", iss)
	}

	if expired, err := idTokenExpired(raw["exp"]); err != nil {
		return nil, fmt.Errorf("socialauth: invalid id token: %w", err)
	} else if expired {
		return nil, fmt.Errorf("socialauth: invalid id token: token expired")
	}
