...
// in fake tests. It mirrors SocialAuthFakeTest's getEnvironmentSetUp.
func setupFakeManager(t *testing.T) *socialauth.Manager {
	t.Helper()
...
func TestFakeDriverWithUser(t *testing.T) {
	...
	user := (&socialauth.User{}).Map(map[string]any{
		"id":    "123",
		"name":  "Test User",
...
func TestFakeDriverWithClosure(t *testing.T) {
	...
	m.FakeWith("github", func() *socialauth.User {
		return (&socialauth.User{}).Map(map[string]any{"id": "456", "name": "Closure User"})
	})
...
func TestFakeMultipleDrivers(t *testing.T) {
	...
	m.Fake("github", (&socialauth.User{}).Map(map[string]any{"id": "github-123"}))
	m.Fake("google", (&socialauth.User{}).Map(map[string]any{"id": "google-456"}))
...
func TestFakeForwardsChainedCalls(t *testing.T) {
	m := setupFakeManager(t)
	fp := m.Fake("github", (&socialauth.User{}).Map(map[string]any{"id": "123"}))
