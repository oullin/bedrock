...
func TestManagerBillableRegistration(t *testing.T) {
	...
	m.Billable("team").
		Resolve(func(r *http.Request) (billing.Billable, error) {
			return stubBillable{id: 1, btype: "team"}, nil
		}).
		Authorize(func(b billing.Billable, r *http.Request) bool {
...
func TestManagerPlans(t *testing.T) {
	...
	m.AddPlan("team", billing.BillingPlan{ID: "pri_monthly", Name: "Pro", Interval: "monthly"})
...
func TestManagerDefaultBillableType(t *testing.T) {
	m := billing.NewManager()
	m.RegisterBillable(billing.BillableConfig{ModelName: "team"})

	if got := m.DefaultBillableType(); got != "team" {
		t.Errorf("DefaultBillableType() = %q, want %q", got, "team")
	}

	m.RegisterBillable(billing.BillableConfig{ModelName: "user"})
...
func TestManagerPerSeatBilling(t *testing.T) {
	...
	m.Billable("team").ChargePerSeat("Team Member", func(b billing.Billable) int { return 5 })
