...
func TestProduct_TypeChecksAndScopes(t *testing.T) {
	sub := &billing.Product{Type: billing.ProductTypeSubscription, Active: true}
	oneTime := &billing.Product{Type: billing.ProductTypeOneTime, Active: true}
	inactive := &billing.Product{Type: billing.ProductTypeSubscription, Active: false}
	...
	products := []*billing.Product{sub, oneTime, inactive}
