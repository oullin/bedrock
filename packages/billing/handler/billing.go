...
type BillingHandler struct {
	...
	resolver billing.BillableResolver
}

// NewBillingHandler creates a BillingHandler.
func NewBillingHandler(b *billing.Workflow, resolver billing.BillableResolver) *BillingHandler {
...
func (h *BillingHandler) StartCheckout(w http.ResponseWriter, r *http.Request) {
	...
	if input.Plan == billing.PlanStarter {
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(map[string]any{
			"errors": []billing.ValidationError{{Field: "plan", Message: "The selected plan pricing is currently unavailable."}},
		})
