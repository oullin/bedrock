...
// SubscriptionHandler handles Billing vendor subscription operations (create,
...
func (h *SubscriptionHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if !billing.ValidPlan(h.manager, billableType, planID) {
	...
	items := []billing.CheckoutItem{{PriceID: planID, Quantity: 1}}
...
func (h *SubscriptionHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	proration := billing.ProratedNextBillingPeriod

	if h.manager.Prorates() {
		proration = billing.ProratedImmediately
	}
...
func (h *SubscriptionHandler) Resume(w http.ResponseWriter, r *http.Request) {
	...
	if h.manager.ChargesPerSeat(billableType) {
		...
		if priceID != "" {
			_ = h.provider.UpdateQuantity(ctx, sub.ProviderID, priceID, quantity, billing.ProratedNextBillingPeriod)
		}
	}
...
func findOrCreateCustomer(ctx context.Context, billable billing.Billable, subs billing.SubscriptionStore) (*billing.Customer, error) {
	_ = ctx
	_ = subs
