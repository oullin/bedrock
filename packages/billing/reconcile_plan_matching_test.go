...
func TestMadoraReconcileSubscriptionPlanMatching(t *testing.T) {
	t.Run("selects the pending subscription matched by price", func(t *testing.T) {
		matching := &billing.Subscription{
			ID:           10,
			BillableType: "team",
			BillableID:   42,
			Plan:         "starter",
			Status:       billing.StatusAwaitingPayment,
			Items:        []billing.SubscriptionItem{{PriceID: "pri_match"}},
		}
		other := &billing.Subscription{
			ID:           11,
			BillableType: "team",
			BillableID:   42,
			Plan:         "pro",
			Status:       billing.StatusAwaitingPayment,
			Items:        []billing.SubscriptionItem{{PriceID: "pri_other"}},
		}
		...
		store := &madoraSubStore{subs: []*billing.Subscription{other, matching, cashier}}
