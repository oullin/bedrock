...
func TestInventoryChannelFactoryReturnsProtocolChannelTypes(t *testing.T) {
	...
	cases := []struct {
		name  string
		check func(contractsWebSockets.Channel) bool
	}{
		{"public-room", func(ch contractsWebSockets.Channel) bool {
			_, ok := ch.(*websockets.Channel)

			return ok
		}},
		{"private-room", func(ch contractsWebSockets.Channel) bool {
			_, ok := ch.(*websockets.PrivateChannel)

			return ok
		}},
		{"presence-room", func(ch contractsWebSockets.Channel) bool {
			_, ok := ch.(*websockets.PresenceChannel)

			return ok
		}},
		{"cache-room", func(ch contractsWebSockets.Channel) bool {
			_, ok := ch.(*websockets.CacheChannel)

			return ok
		}},
		{"private-cache-room", func(ch contractsWebSockets.Channel) bool {
			_, ok := ch.(*websockets.PrivateCacheChannel)

			return ok
		}},
		{"presence-cache-room", func(ch contractsWebSockets.Channel) bool {
			_, ok := ch.(*websockets.PresenceCacheChannel)
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if ch := websockets.NewChannel(tc.name, app); !tc.check(ch) {
...
func TestInventoryPublicChannelLifecycle(t *testing.T) {
	...
	app := newTestApp()
	apps := websockets.NewAppManager([]websockets.AppConfig{{ID: app.ID(), Key: app.Key(), Secret: app.Secret()}})
	manager := websockets.NewChannelManager(apps)
	...
	event := contractsWebSockets.Event{Event: "inventory-event", Data: `{"ok":true}`, Channel: ch.Name()}
	...
	if err := ch.Broadcast(ctx, contractsWebSockets.Event{Event: "receiver-only", Data: `{}`, Channel: ch.Name()}, &except); err != nil {
...
func TestInventoryPrivateChannelLifecycle(t *testing.T) {
	...
	privateChannel := websockets.NewPrivateChannel(privateName, app)
	...
	if err := privateChannel.BroadcastToAll(ctx, contractsWebSockets.Event{Event: "private-event", Data: `{}`, Channel: privateName}); err != nil {
	...
	if err := websockets.NewPrivateChannel(privateName, app).Subscribe(ctx, newFakeConn("socket-2", app.ID()), "bad-signature", ""); err != websockets.ErrUnauthorized {
		t.Fatalf("invalid private signature error = %v, want ErrUnauthorized", err)
	}

	if err := websockets.NewPrivateChannel(privateName, app).Subscribe(ctx, newFakeConn("socket-3", app.ID()), "", ""); err != websockets.ErrUnauthorized {
		t.Fatalf("missing private auth error = %v, want ErrUnauthorized", err)
	}

	if err := websockets.NewPresenceChannel("presence-room", app).Subscribe(ctx, newFakeConn("socket-4", app.ID()), "", `{"user_id":"1"}`); err != websockets.ErrUnauthorized {
...
func TestInventoryCacheChannels(t *testing.T) {
	...
	cacheChannel := websockets.NewCacheChannel("cache-room", app)
	...
	cached := contractsWebSockets.Event{Event: "cached-event", Data: `{"cached":true}`, Channel: "cache-room"}
	...
	privateCache := websockets.NewPrivateCacheChannel(privateName, app)
	...
	if err := privateCache.BroadcastToAll(ctx, contractsWebSockets.Event{Event: "private-cached", Data: `{}`, Channel: privateName}); err != nil {
	...
	if err := websockets.NewPrivateCacheChannel(privateName, app).Subscribe(ctx, newFakeConn("socket-3", app.ID()), "bad-signature", ""); err != websockets.ErrUnauthorized {
...
func TestInventoryPresenceChannelLifecycle(t *testing.T) {
	...
	channel := websockets.NewPresenceChannel(channelName, app)
	...
	if err := channel.BroadcastToAll(ctx, contractsWebSockets.Event{Event: "presence-event", Data: `{}`, Channel: channelName}); err != nil {
	...
	if err := websockets.NewPresenceChannel(channelName, app).Subscribe(ctx, newFakeConn("socket-4", app.ID()), "bad-signature", aliceData); err != websockets.ErrUnauthorized {
...
func TestInventoryPresenceCacheChannelLifecycle(t *testing.T) {
	...
	channel := websockets.NewPresenceCacheChannel(channelName, app)
	...
	event := contractsWebSockets.Event{Event: "presence-cache-event", Data: `{}`, Channel: channelName}
	...
	if err := websockets.NewPresenceCacheChannel(channelName, app).Subscribe(ctx, newFakeConn("socket-3", app.ID()), "bad-signature", aliceData); err != websockets.ErrUnauthorized {
...
func TestInventoryChannelManagerOperations(t *testing.T) {
	...
	ctx := context.Background()
	apps := websockets.NewAppManager([]websockets.AppConfig{{ID: "app-1", Key: "key-1", Secret: "secret-1"}})
	manager := websockets.NewChannelManager(apps)
...
func TestInventoryPusherMessageFormattingAndParsing(t *testing.T) {
	...
	payload, err := websockets.MarshalEvent("inventory-event", "public-room", map[string]any{"answer": 42})
	...
	errorPayload, err := websockets.MarshalError(websockets.CodeInvalidMessage, "bad message")
	...
	subscribe, err := websockets.Parse([]byte(`{"event":"pusher:subscribe","data":{"channel":"public-room"}}`))

	if err != nil || subscribe.Event != "pusher:subscribe" {
		t.Fatalf("unexpected subscribe parse: msg=%+v err=%v", subscribe, err)
	}

	subscribeData, err := websockets.ParseSubscribeData(subscribe.Data)
	...
	unsubscribe, err := websockets.Parse([]byte(`{"event":"pusher:unsubscribe","data":{"channel":"public-room"}}`))

	if err != nil || unsubscribe.Event != "pusher:unsubscribe" {
		t.Fatalf("unexpected unsubscribe parse: msg=%+v err=%v", unsubscribe, err)
	}

	ping, err := websockets.Parse([]byte(`{"event":"pusher:ping","data":{}}`))
...
func TestInventoryHTTPPusherEndpoints(t *testing.T) {
	...
	batchBody, _ := json.Marshal(websockets.BatchTriggerRequest{Batch: []websockets.TriggerRequest{
...
func TestInventorySyncDispatcherBroadcastsEvents(t *testing.T) {
	...
	apps := websockets.NewAppManager([]websockets.AppConfig{{ID: "app-1", Key: "key-1", Secret: "secret-1"}})
	manager := websockets.NewChannelManager(apps)
	dispatcher := websockets.NewSyncDispatcher(manager)
	...
	if err := dispatcher.Dispatch(ctx, "app-1", contractsWebSockets.Event{Event: "published", Data: `{}`, Channel: "public-one"}); err != nil {
		t.Fatalf("dispatch first: %v", err)
	}

	if err := dispatcher.Dispatch(ctx, "app-1", contractsWebSockets.Event{Event: "published-two", Data: `{}`, Channel: "public-two"}); err != nil {
	...
	if err := first.BroadcastToAll(ctx, contractsWebSockets.Event{Event: "direct-broadcast", Data: `{}`, Channel: "public-one"}); err != nil {
...
func TestInventoryServerConfigurationPrimitives(t *testing.T) {
	...
	if websockets.ValidateOrigin("https://app.example.com", []string{"*.example.com"}) != true {
		t.Fatalf("expected wildcard origin to be valid")
	}

	if websockets.ValidateOrigin("https://evil.test", []string{"*.example.com"}) != false {
		t.Fatalf("expected unmatched origin to be invalid")
	}

	apps := websockets.NewAppManager([]websockets.AppConfig{{ID: "app-1", Key: "key-1", Secret: "secret-1", MaxConnections: 1, MaxMessageSize: 32}})
	...
	connections := websockets.NewConnectionManager()
	channels := websockets.NewChannelManager(apps)
	server := websockets.NewServer(websockets.DefaultConfig(), apps, connections, channels, websockets.NewSyncDispatcher(channels))
	...
	connections.Add(websockets.NewConn(nil, "app-1"))
