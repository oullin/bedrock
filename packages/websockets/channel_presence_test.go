...
func TestPresenceChannel_Subscribe_InvalidAuth(t *testing.T) {
	...
	app := newTestApp()
	ch := websockets.NewPresenceChannel("presence-test", app)
...
func TestPresenceChannel_Subscribe_ValidAuth(t *testing.T) {
	...
	sig := websockets.SignChannel("secret-1", socketID, channelName, channelData)
	auth := "key-1:" + sig

	ch := websockets.NewPresenceChannel(channelName, app)
...
func TestPresenceChannel_MemberAdded_Broadcast(t *testing.T) {
	...
	sig1 := websockets.SignChannel("secret-1", sock1, channelName, data1)
	auth1 := "key-1:" + sig1
	...
	sig2 := websockets.SignChannel("secret-1", sock2, channelName, data2)
	auth2 := "key-1:" + sig2
