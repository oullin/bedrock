...
func newWebSocketHarness(t *testing.T, cfg websockets.AppConfig) (*httptest.Server, *websockets.App, *websockets.ChannelManager, *websockets.SyncDispatcher) {
	ts, app, _, _, channels, dispatcher := newWebSocketHarnessWithManagers(t, cfg)

	return ts, app, channels, dispatcher
}

func newWebSocketHarnessWithManagers(t *testing.T, cfg websockets.AppConfig) (*httptest.Server, *websockets.App, *websockets.AppManager, *websockets.ConnectionManager, *websockets.ChannelManager, *websockets.SyncDispatcher) {
	t.Helper()
	...
	server := websockets.NewServer(websockets.DefaultConfig(), apps, conns, channels, dispatcher)
	handler := websockets.NewHTTPHandler(apps, conns, channels, dispatcher)
...
func waitForChannelRemoval(t *testing.T, channels *websockets.ChannelManager, appID, name string) {
	t.Helper()
...
func TestInventoryServerClientEventPolicies(t *testing.T) {
	...
	t.Run("all forwards public client events with empty data", func(t *testing.T) {
		...
		ts, app, _, _ := newWebSocketHarness(t, websockets.AppConfig{
			ID:             "app-1",
			Key:            "key-1",
			Secret:         "secret-1",
	...
	t.Run("none rejects client events", func(t *testing.T) {
		...
		ts, app, _, _ := newWebSocketHarness(t, websockets.AppConfig{
			...
			Secret:         "secret-1",
			AllowedOrigins: []string{"*"},
			ClientEvents:   websockets.ClientEventsConfig{Mode: "none"},
		})
	...
	t.Run("members rejects public channel client events", func(t *testing.T) {
		...
		ts, app, _, _ := newWebSocketHarness(t, websockets.AppConfig{
			...
			Secret:         "secret-1",
			AllowedOrigins: []string{"*"},
			ClientEvents:   websockets.ClientEventsConfig{Mode: "members"},
		})
...
func TestInventoryServerInactiveConnectionJobs(t *testing.T) {
	...
	ts, app, apps, conns, _, _ := newWebSocketHarnessWithManagers(t, websockets.AppConfig{
		ID:              "app-1",
		Key:             "key-1",
		Secret:          "secret-1",
		AllowedOrigins:  []string{"*"},
	...
	websockets.PingInactiveConnections(ctx, conns, apps)
...
func TestInventoryServerPublicWebSocketFlow(t *testing.T) {
	...
	ts, app, channels, dispatcher := newWebSocketHarness(t, websockets.AppConfig{
		ID:             "app-1",
		Key:            "key-1",
		Secret:         "secret-1",
		AllowedOrigins: []string{"*"},
	...
	if err := dispatcher.Dispatch(senderCtx, app.ID(), contractsWebSockets.Event{
	...
	triggerBody, _ := json.Marshal(websockets.TriggerRequest{
...
func TestInventoryServerMultiAppAndMultiChannelFlow(t *testing.T) {
	...
	apps := websockets.NewAppManager([]websockets.AppConfig{app1, app2})
	channels := websockets.NewChannelManager(apps)
	conns := websockets.NewConnectionManager()
	dispatcher := websockets.NewSyncDispatcher(channels)
	server := websockets.NewServer(websockets.DefaultConfig(), apps, conns, channels, dispatcher)
...
func TestInventoryServerPresenceAndCacheJoinFlow(t *testing.T) {
	...
	if err := dispatcher.Dispatch(aliceCtx, app.ID(), contractsWebSockets.Event{
		Event:   "cache-event",
		Data:    `{"cached":true}`,
		Channel: cacheName,
	}); err != nil {
