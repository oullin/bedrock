...
type ChannelManager struct {
	mu       sync.RWMutex
	channels map[string]map[string]contractsWebSockets.Channel // appID → channelName → Channel
	apps     *AppManager
}
...
func NewChannelManager(apps *AppManager) *ChannelManager {
	return &ChannelManager{
		channels: make(map[string]map[string]contractsWebSockets.Channel),
		apps:     apps,
	}
}
...
func (m *ChannelManager) GetOrCreate(appID, channelName string) (contractsWebSockets.Channel, error) {
	...
	if _, ok := m.channels[appID]; !ok {
		m.channels[appID] = make(map[string]contractsWebSockets.Channel)
	}
...
func (m *ChannelManager) Get(appID, channelName string) (contractsWebSockets.Channel, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *ChannelManager) All(appID string) []contractsWebSockets.Channel {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	out := make([]contractsWebSockets.Channel, 0, len(appChans))
