...
func newArrayInventoryHarness(dispatcher *testDispatcher) (*pennant.ArrayDriver, *pennant.Decorator) {
	driver := pennant.NewArrayDriverWithDispatcher(dispatcher)

	return driver, pennant.NewDecoratorWithDispatcher(driver, dispatcher)
}

func newDatabaseInventoryHarness(dispatcher *testDispatcher) (*inMemoryDB, *pennant.DatabaseDriver, *pennant.Decorator) {
	db := newDB()
	driver := pennant.NewDatabaseDriverWithDispatcher(db, testTable, dispatcher)

	return db, driver, pennant.NewDecoratorWithDispatcher(driver, dispatcher)
}

func requireFeatureValue(t *testing.T, dec *pennant.Decorator, feature string, scope any) any {
	t.Helper()
...
func requireFeatureActive(t *testing.T, interaction *pennant.ScopedFeatureInteraction, feature string) {
	t.Helper()
...
func requireFeatureInactive(t *testing.T, interaction *pennant.ScopedFeatureInteraction, feature string) {
	t.Helper()
...
func requireStoredFeature(t *testing.T, dec *pennant.Decorator, feature string) {
	t.Helper()
...
func TestInventoryParityArrayDriverFeatureLifecycle(t *testing.T) {
	...
	if _, err := dec.For("user:1", "user:2").Value(ctx, "scope-value"); !errors.Is(err, pennant.ErrMultipleScopes) {
	...
	dec.DefineValue("lottery-on", pennant.LotteryOdds(1, 1))
	dec.DefineValue("lottery-off", pennant.LotteryOdds(0, 1))
	dec.Define("lottery-from-resolver", func(context.Context, any) (any, error) {
		return pennant.LotteryOdds(0, 1), nil
	})
...
func TestInventoryParityDatabaseDriverFeatureLifecycle(t *testing.T) {
	...
	serialized, err := pennant.SerializeScope(inventoryScope{id: "42"})
...
func TestInventoryParityDatabaseDriverPurgingEventsAndRetries(t *testing.T) {
	...
	if !errors.Is(err, pennant.ErrStorageConflict) {
...
func TestInventoryParityDatabaseDriverRepositoryEdges(t *testing.T) {
	...
	if err := dec.SetAll(ctx, []pennant.FeatureEntry{{Feature: "timestamped", Scope: nil, Value: true}}); err != nil {
	...
	errorDriver := pennant.NewDatabaseDriver(errorDB, testTable)
...
func TestInventoryParityFeatureManagerAndScopeSerialization(t *testing.T) {
	...
	ctx := context.Background()
	manager := pennant.NewManagerWithDispatcher("array", &testDispatcher{})
	...
	lottery := pennant.FixedLottery(true, true, true, true, false)
...
func TestInventoryParityFeatureMiddleware(t *testing.T) {
	...
	dispatcher := &testDispatcher{}
	_, dec := newArrayInventoryHarness(dispatcher)
	middleware := pennant.NewEnsureFeaturesAreActive(dec.For())
	...
	if got := middleware.Using("test", "another"); got != "pennant.EnsureFeaturesAreActive:test,another" {
