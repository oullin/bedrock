# Upstream Prompts Parity — Adaptation Rules

This document governs how `packages/prompts` maintains behavioural parity with
`upstream/prompts` and its Pest test suite.

**Source of truth:** https://github.com/upstream/prompts
...
```go
// Port of Upstream\Prompts\Tests\Feature\{TestClass}::{testMethod}
...
| Upstream validation rules (`required\|email`) | `ValidateFunc` closure returning error message string            |
