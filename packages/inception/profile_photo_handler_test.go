...
func TestUpdateProfilePhotoSuccess(t *testing.T) {
	...
	app.config.Features.ProfilePhotos = true

	handler := NewUpdateProfilePhotoHandler(app, photos)
...
func TestUpdateProfilePhotoDisabled(t *testing.T) {
	...
	app.config.Features.ProfilePhotos = false

	handler := NewUpdateProfilePhotoHandler(app, &testUpdatesPhotos{})
	w := httptest.NewRecorder()
...
func TestDeleteProfilePhotoSuccess(t *testing.T) {
	...
	handler := NewDeleteProfilePhotoHandler(app, photos)
	w := httptest.NewRecorder()
...
func TestDeleteAccountSuccess(t *testing.T) {
	...
	handler := NewDeleteAccountHandler(app, deletes)
	w := httptest.NewRecorder()
	...
	if len(evts.dispatched) != 1 || evts.dispatched[0].(Event).Name != EventUserDeleted {
...
func TestDeleteAccountDisabled(t *testing.T) {
	app, _ := buildTestInceptionJS(&testTeamUser{id: "1"}, newTestTeamRepo(), newTestInvitationRepo())
	app.config.Features.AccountDeletion = false

	handler := NewDeleteAccountHandler(app, &testDeletesUsers{})
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodDelete, "/user", nil)
