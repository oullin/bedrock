...
func TestCreateTokenSuccess(t *testing.T) {
	...
	app, events := buildTestInceptionJS(user, newTestTeamRepo(), newTestInvitationRepo())

	handler := NewCreateTokenHandler(app, tokenRepo)
...
func TestCreateTokenMissingName(t *testing.T) {
	...
	handler := NewCreateTokenHandler(app, newTestTokenRepo())
	w := httptest.NewRecorder()
...
func TestUpdateTokenSuccess(t *testing.T) {
	...
	mux.Handle("PUT /user/api-tokens/{token}", NewUpdateTokenHandler(app, tokenRepo))
...
func TestDeleteTokenSuccess(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.Handle("DELETE /user/api-tokens/{token}", NewDeleteTokenHandler(app, tokenRepo))
