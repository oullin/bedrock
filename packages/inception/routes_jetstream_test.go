...
func buildFullInceptionJS() (*Inception, *testTeamUser, RouteConfig) {
...
func TestRegisterJetstreamRoutesAllFeatures(t *testing.T) {
	...
	RegisterRoutes(router, app, "test-app", config)
...
func TestFullTeamLifecycle(t *testing.T) {
	app, user, config := buildFullInceptionJS()
	...
	teamRepo := app.teams.(*testTeamRepo)
