// Package openai provides the OpenAI concrete provider implementation.
// OpenAI supports: Text, Image, Audio, Embeddings, Transcription, Files, and Vector Stores.
package openai
...
// Provider implements the OpenAI AI provider.
// It supports text, image, audio, embeddings, transcription, file, and store capabilities.
...
// NewProvider constructs an OpenAI provider with the given config.
func NewProvider(config map[string]any) *Provider {
	return &Provider{Provider: providers.NewProvider("openai", config)}
}
...
func (p *Provider) SmartestTextModel() string { return "o3" }
