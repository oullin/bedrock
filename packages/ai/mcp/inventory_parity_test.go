...
// Ports of laravel/mcp inventory entries whose behavior is already present in
// the Go MCP package. These tests keep exact upstream markers next to executable
// assertions so services/scripts/laravel-compliance.sh can count the parity.
...
func TestInventoryJSONRPCRequestAndTransportParity(t *testing.T) {
	...
	req, err := mcp.ParseJsonRpcRequest([]byte(`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"echo","_meta":{"trace":"abc"},"arguments":{"text":"hi"}}}`), "sid")
	if err != nil {
		t.Fatalf("unexpected parse error: %v", err)
	}
	if req.Method != "tools/call" || req.SessionID != "sid" || req.Get("name") != "echo" {
...
func TestInventoryServerMethodParity(t *testing.T) {
	...
	srv.Test(t).CallTool("echo", map[string]any{"text": "hello"}).AssertOK().AssertSee("hello")
...
func TestInventoryCursorUriAndHTTPParity(t *testing.T) {
	...
	resp = httptest.NewRecorder()
	body = strings.NewReader(`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"echo","arguments":{"text":"http"}}}`)
	req = httptest.NewRequest(http.MethodPost, "/mcp", body)
	srv.ServeHTTP(resp, req)
