...
func TestParse_MissingEvent(t *testing.T) {
	...
	_, err := reverb.Parse(raw)
	if !errors.Is(err, reverb.ErrInvalidMessage) {
...
func TestMarshalEvent_IncludesChannel(t *testing.T) {
	...
	b, err := reverb.MarshalEvent("pusher:ping", "my-channel", map[string]string{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestMarshalEvent_OmitsEmptyChannel(t *testing.T) {
	...
	b, err := reverb.MarshalEvent("pusher:ping", "", map[string]string{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestMarshalError_CorrectCode(t *testing.T) {
	...
	b, err := reverb.MarshalError(reverb.CodeInvalidMessage, "bad message")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if inner.Code != reverb.CodeInvalidMessage {
		t.Errorf("expected code %d, got %d", reverb.CodeInvalidMessage, inner.Code)
	}
...
func TestParseSubscribeData_DirectObject(t *testing.T) {
	...
	sd, err := reverb.ParseSubscribeData(raw)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
