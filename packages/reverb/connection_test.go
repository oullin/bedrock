...
func TestConnectionManager_Add_Find(t *testing.T) {
	...
	mgr := reverb.NewConnectionManager()
...
func TestConnectionManager_Count(t *testing.T) {
	...
	mgr := reverb.NewConnectionManager()
	c1 := reverb.NewConn(nil, "app-1")
	c2 := reverb.NewConn(nil, "app-1")
	c3 := reverb.NewConn(nil, "app-1")
...
func TestConnectionManager_MultipleApps(t *testing.T) {
	...
	mgr := reverb.NewConnectionManager()
	connA := reverb.NewConn(nil, "app-a")
	connB := reverb.NewConn(nil, "app-b")
