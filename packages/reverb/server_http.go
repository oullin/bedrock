...
func NewHTTPHandler(apps *AppManager, conns *ConnectionManager, channels *ChannelManager, dispatcher contractsReverb.Dispatcher) *HTTPHandler {
	return &HTTPHandler{
		apps:       apps,
		conns:      conns,
		channels:   channels,
...
func projectChannelListInfo(ch contractsReverb.Channel, info map[string]struct{}) map[string]any {
	if len(info) == 0 {
	...
	if hasInfo(info, "user_count") {
		if pc, ok := ch.(contractsReverb.PresenceChanneler); ok {
			attrs["user_count"] = pc.MemberCount()
		}
	}

	if hasInfo(info, "cache") {
		if cache, ok := ch.(contractsReverb.CacheableChannel); ok {
...
func projectChannelInfo(ch contractsReverb.Channel, info map[string]struct{}) map[string]any {
	if len(ch.Connections()) == 0 {
...
func cachedEventData(event *contractsReverb.Event) any {
	if event == nil {
		return nil
	}
...
func projectEventInfo(chanName string, ch contractsReverb.Channel, info map[string]struct{}) map[string]any {
	attrs := make(map[string]any)
