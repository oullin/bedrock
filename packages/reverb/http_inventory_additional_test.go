...
func signedRequestWithExtraParams(t *testing.T, method, path, secret string, body []byte, extra map[string]string) *http.Request {
	...
	sig := reverb.SignHTTPRequest(secret, method, path, params)
...
func TestInventoryHTTPHealthConnectionsAndCustomPath(t *testing.T) {
	...
	conns.Add(reverb.NewConn(nil, "app-1"))
	conns.Add(reverb.NewConn(nil, "app-1"))
	...
	values.Set("auth_signature", reverb.SignHTTPRequest("secret-1", http.MethodPost, "/apps/app-1/events", params))
...
func TestInventoryHTTPEventInfoResponses(t *testing.T) {
	...
	app, err := reverb.NewAppManager([]reverb.AppConfig{{ID: "app-1", Key: "key-1", Secret: "secret-1"}}).FindByID("app-1")
	if err != nil {
