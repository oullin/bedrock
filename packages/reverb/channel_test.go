...
func TestChannel_Subscribe_Public(t *testing.T) {
	...
	app := newTestApp()
	ch := reverb.NewChannel("public-test", app)
...
func TestChannel_Subscribe_Private_ValidAuth(t *testing.T) {
	...
	sig := reverb.SignChannel("test-secret", socketID, channelName, "")
	auth := "key-1:" + sig

	ch := reverb.NewPrivateChannel(channelName, app)
...
func TestPrivateChannel_Subscribe_InvalidAuth(t *testing.T) {
	...
	app := newTestApp()
	ch := reverb.NewPrivateChannel("private-foo", app)
	conn := newFakeConn("sock-1", app.ID())
	ctx := context.Background()
