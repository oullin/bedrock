...
func (ch *PrivateChannel) Subscribe(ctx context.Context, conn contractsReverb.Connection, auth, data string) error {
...
// Broadcast sends the event to all subscribers except the sender.
func (ch *PrivateChannel) Broadcast(ctx context.Context, event contractsReverb.Event, except *string) error {
	return ch.broadcast(ctx, event, except)
}

// BroadcastToAll sends the event to every subscriber.
func (ch *PrivateChannel) BroadcastToAll(ctx context.Context, event contractsReverb.Event) error {
	return ch.broadcastAll(ctx, event)
}
...
func NewChannel(name string, app *App) contractsReverb.Channel {
	switch {
	case strings.HasPrefix(name, "private-cache-"):
