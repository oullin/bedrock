...
func TestScopeFields(t *testing.T) {
	s := passport.Scope{ID: "orders:read", Description: "Read orders"}
...
func TestScopeToArray(t *testing.T) {
	s := passport.Scope{ID: "user:read", Description: "Read user data"}
	got := s.ToArray()
...
func TestScopeToJSON(t *testing.T) {
	s := passport.Scope{ID: "admin", Description: "Admin access"}
