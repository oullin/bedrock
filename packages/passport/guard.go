...
//
// It implements contracts/auth.Guard and mirrors Passport TokenGuard.
...
func (g *TokenGuard) User(ctx context.Context) (cauth.Authenticatable, error) {
	...
	// 1. Test override.
	if actingUser, actingToken, _, _, ok := g.passport.actingAsState(); ok && actingUser != nil {
		if g.user == nil {
	...
	accessToken := NewAccessToken(token, g.passport)
