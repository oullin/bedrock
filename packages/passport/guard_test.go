...
func newGuard(p *passport.Passport, tokens passport.TokenStore, clients passport.ClientStore, users map[string]cauth.Authenticatable) *passport.TokenGuard {
	...
	return passport.NewTokenGuard(p, tokens, clients, provider)
}
...
func TestTokenGuardReturnsNilForRevokedToken(t *testing.T) {
	p := newPassport()
	tokens := passport.NewMemoryTokenStore()
	clients := passport.NewMemoryClientStore()
...
func TestTokenGuardClientReturnsClientForToken(t *testing.T) {
	...
	client := &passport.Client{ID: "c1", Name: "MyApp", Secret: "s"}
...
func TestTokenGuardClientResolvesBearerTokenWithoutUserResolution(t *testing.T) {
	...
	client := &passport.Client{ID: "c1", Name: "Worker", Secret: "s"}
	_ = clients.Create(context.Background(), client)
...
func TestTokenGuardActingAsClientOverride(t *testing.T) {
	p := newPassport()
	client := &passport.Client{ID: "c99", Name: "TestClient"}
