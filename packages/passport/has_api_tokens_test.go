...
func TestTokenCanReturnsTrueForPresentScope(t *testing.T) {
	...
	at := passport.NewAccessToken(tok, newPassport())

	user := newStubUser("u1")
	uwt := passport.NewUserWithTokens(user, at)
...
func TestUserWithTokensWrapsAuthenticatable(t *testing.T) {
	...
	uwt := passport.NewUserWithTokens(user, nil)
...
func TestHasApiTokensInterface(t *testing.T) {
	...
	var _ passport.HasApiTokens = uwt
}
