...
// WithPassport attaches the Passport config to this token so that scope
// resolution (including inherited scopes) can be performed via Can/Cant.
func (t *Token) WithPassport(p *Passport) *Token {
	t.passport = p
...
func (t *Token) Can(scope string) bool {
	...
	if t.passport != nil {
		inherited = t.passport.InheritedScopesEnabled()
	}
