...
func TestTokenCanReturnsTrueForMatchingScope(t *testing.T) {
	...
	tok = tok.WithPassport(newPassport())
...
func TestTokenWithoutPassportUsesDirectScopeMatch(t *testing.T) {
	tok := &passport.Token{
		Scopes: []string{"user"},
	}

	if !tok.Can("user") {
		t.Error("Can should still work without a passport attached")
	}

	if tok.Can("user:read") {
		t.Error("Can(\"user:read\") should be false without a passport (no inherited scope resolution)")
	}
}
