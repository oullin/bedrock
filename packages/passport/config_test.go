...
func TestLoadConfigReadsPrivateKey(t *testing.T) {
	repo := config.New(map[string]any{
		"passport.private_key": "-----BEGIN RSA PRIVATE KEY-----\nMIIEow==\n-----END RSA PRIVATE KEY-----",
	})

	cfg := passport.LoadConfig(repo)
...
func TestLoadConfigReadsPublicKey(t *testing.T) {
	repo := config.New(map[string]any{
		"passport.public_key": "-----BEGIN PUBLIC KEY-----\nMIIBIjAN\n-----END PUBLIC KEY-----",
	})
...
func TestLoadConfigReadsPersonalAccessClient(t *testing.T) {
	repo := config.New(map[string]any{
		"passport.personal_access_client.id":     "client-uuid-123",
		"passport.personal_access_client.secret": "super-secret-value",
	})
...
func TestLoadConfigReadsStorageDatabase(t *testing.T) {
	repo := config.New(map[string]any{
		"passport.storage.database": "passport_db",
	})
