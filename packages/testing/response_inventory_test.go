...
func TestResponseInventoryFailures(t *testing.T) {
	...
	t.Run("TestResponseTest::testAssertHTTPPreviewSuccessfulWithIncorrectValue", func(t *testing.T) {
		rec := newRecorder()
		rec.Header().Set("HTTPPreview-Success", "false")

		expectMockFailure(t, func(m *mockTB) {
			packagetesting.AssertResponse(m, rec).HTTPPreviewSuccessful()
		})
	})

	t.Run("TestResponseTest::testAssertHTTPPreviewSuccessfulWithMissingHeader", func(t *testing.T) {
		rec := newRecorder()
