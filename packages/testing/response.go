...
// Assertions wraps an httptest.ResponseRecorder with Laravel-style response
// assertions.
...
func (a *Assertions) PrecognitionSuccessful() *Assertions {
	...
	if got := a.rec.Header().Get("Precognition-Success"); got != "true" {
		a.t.Fatalf("expected Precognition-Success header to be true, got %q", got)
	}
