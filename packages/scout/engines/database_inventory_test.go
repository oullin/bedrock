...
func TestDatabaseEngineInventoryEmptySearch(t *testing.T) {
	...
	result, err := engine.Search(context.Background(), scout.NewBuilder(model, ""))
...
func TestDatabaseEngineInventorySearchClauseAndResults(t *testing.T) {
	...
	engine := engines.NewDatabaseEngine(&mockResolver{conn: conn})
	model := newTestModelWithData(0, "posts", map[string]any{"id": 0, "title": ""})
	result, err := engine.Search(context.Background(), scout.NewBuilder(model, "Go"))
...
func TestDatabaseEngineInventoryPaginationAndLimit(t *testing.T) {
	...
	engine := engines.NewDatabaseEngine(&mockResolver{conn: conn})
	model := newTestModelWithData(0, "posts", map[string]any{"id": 0, "title": ""})
	_, err := engine.Paginate(context.Background(), scout.NewBuilder(model, "Alpha"), 15, 3)
	...
	_, err = engine.Search(context.Background(), scout.NewBuilder(model, "Alpha").Take(5))
...
func TestDatabaseEngineInventoryOrderingAndSearchColumns(t *testing.T) {
	...
	builder := scout.NewBuilder(model, "Alpha").
	...
	if !strings.Contains(conn.lastSQL, "title like ?") || strings.Contains(conn.lastSQL, "body like ?") {
		t.Fatalf("expected custom scout query columns, got SQL: %s", conn.lastSQL)
	}
...
func TestDatabaseEngineInventoryWhereComparisons(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "greater than",
			builder:   scout.NewBuilder(newTestModelWithData(0, "posts", map[string]any{"id": 0}), "").Where("views", ">", 10),
		...
		},
		{
			...
			builder:   scout.NewBuilder(newTestModelWithData(0, "posts", map[string]any{"id": 0}), "").Where("views", ">", 5).Where("id", "!=", 2),
