...
func TestFortifyViewsDisabledAndRouteOverrides(t *testing.T) {
	t.Parallel()
	...
	env := newFortifyEnv(t, func(repo *configpkg.Repository) {
		repo.Set(map[string]any{
			"fortify.paths.login": "/sign-in",
			"fortify.views":       true,
		})
	}, func(deps *fortify.Dependencies) {
	...
	envNoViews := newFortifyEnv(t, func(repo *configpkg.Repository) {
		repo.Set("fortify.views", false)
	}, nil)
...
func TestFortifyConfirmedPasswordStatusBranches(t *testing.T) {
	t.Parallel()

	env := newFortifyEnv(t, nil, nil)
...
func TestFortifyTwoFactorConfirmationAndChallengeFlow(t *testing.T) {
	t.Parallel()
...
func TestFortifyPasswordResetFailureModes(t *testing.T) {
	t.Parallel()
...
func newFortifyEnv(t *testing.T, mutateRepo func(*configpkg.Repository), mutateDeps func(*fortify.Dependencies)) fortifyEnv {
	...
	server, err := fortify.NewServerFromRepository(repo, deps)
