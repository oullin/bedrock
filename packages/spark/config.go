...
// Config is Spark's typed view over the shared Bedrock config repository.
type Config struct {
...
// DefaultConfigRepository returns the canonical config repository with Spark
// defaults registered under the "spark" namespace.

// DefaultConfig returns the typed Spark config backed by the shared repository.

// NewConfig wraps a shared config repository as Spark configuration.

// NewConfigFromValues returns a Spark config with the supplied values layered
...
// FeatureFlags returns the configured Spark feature flags.
...
func DefaultConfigRepository() *config.Repository {
	return config.NewWithDefaults(map[string]any{
		...
		"spark.sandbox":            false,
		"spark.client_side_token":  "",
		"spark.seller_id":          0,
		"spark.retain_key":         "",
		"spark.billables":          map[string]BillableConfig{},
		"spark.feature_flags":      map[string]FeatureConfig{},
	})
}
