...
type fakeCustomerStore struct {
	...
	createErr  error
	savedCalls int
	savedLast  *spark.Customer
}

type fakeTransactionStore struct {
	txns []spark.Transaction
	err  error
}

type fakePaymentReporter struct {
	last, next *spark.Payment
	lastErr    error
	nextErr    error
}
...
func (f *fakeSubStore) FindByID(context.Context, int64) (*spark.Subscription, error) {
	return nil, nil
}
func (f *fakeSubStore) FindByProviderID(context.Context, string) (*spark.Subscription, error) {
	return nil, nil
}
func (f *fakeSubStore) CurrentForBillable(context.Context, string, int64) (*spark.Subscription, error) {
	return f.sub, f.err
}
func (f *fakeSubStore) ActiveForBillable(context.Context, string, int64) ([]*spark.Subscription, error) {
	return nil, nil
}
func (f *fakeSubStore) Create(context.Context, *spark.Subscription) error { return nil }
func (f *fakeSubStore) Save(context.Context, *spark.Subscription) error   { return nil }
func (f *fakeSubStore) Delete(context.Context, int64) error               { return nil }

func (f *fakeCustomerStore) FindByProviderID(context.Context, string) (*spark.Customer, error) {
	return nil, nil
}
func (f *fakeCustomerStore) FindByBillable(context.Context, string, int64) (*spark.Customer, error) {
	return f.customer, f.findErr
}
func (f *fakeCustomerStore) Create(_ context.Context, c *spark.Customer) error {
	f.customer = c

	return f.createErr
}
func (f *fakeCustomerStore) Save(_ context.Context, c *spark.Customer) error {
	f.savedCalls++
	f.savedLast = c
...
func (f *fakeTransactionStore) FindByProviderID(context.Context, string) (*spark.Transaction, error) {
	return nil, nil
}
func (f *fakeTransactionStore) FindByBillable(context.Context, string, int64, int) ([]spark.Transaction, error) {
	return f.txns, f.err
}
func (f *fakeTransactionStore) Create(context.Context, *spark.Transaction) error { return nil }
func (f *fakeTransactionStore) Save(context.Context, *spark.Transaction) error   { return nil }

func (f *fakePaymentReporter) LastPayment(context.Context, *spark.Subscription) (*spark.Payment, error) {
	return f.last, f.lastErr
}
func (f *fakePaymentReporter) NextPayment(context.Context, *spark.Subscription) (*spark.Payment, error) {
...
func TestPlanForSubscription(t *testing.T) {
	planA := &spark.Plan{ID: "pri_a", Name: "A"}
	planB := &spark.Plan{ID: "pri_b", Name: "BName", Options: map[string]any{"slug": "b-slug"}}

	plans := []*spark.Plan{planA, planB}
	...
	tests := []struct {
		...
		{name: "nil subscription", sub: nil, want: nil},
		{
			...
			sub: &spark.Subscription{
				Items: []spark.SubscriptionItem{{PriceID: "pri_a"}},
			},
			want: planA,
		},
		{
			name: "match by slug",
			sub:  &spark.Subscription{Plan: "b-slug"},
			want: planB,
		},
		{
			name: "match by name",
			sub:  &spark.Subscription{Plan: "BName"},
			want: planB,
		},
		{
			name: "no match",
			sub:  &spark.Subscription{Plan: "nope"},
			want: nil,
		},
	}
...
func TestPlansToMaps_SkipsNils(t *testing.T) {
	plans := []*spark.Plan{nil, {ID: "pri_a", Name: "A"}, nil}
...
func TestPlanToMap(t *testing.T) {
	...
	plan := &spark.Plan{ID: "pri", Name: "Plan"}
...
func TestPaymentToMap(t *testing.T) {
	...
	payment := &spark.Payment{Amount: 1200, Currency: "USD", Date: time.Date(2026, 5, 2, 0, 0, 0, 0, time.UTC)}
...
func TestBrandColor_AppName_DashboardURL_DateFormat_Defaults(t *testing.T) {
	...
	f := NewFrontendState(spark.NewManager(), cfg, &fakeSubStore{})

	if f.brandColor() != "bg-gray-800" {
		t.Fatalf("brandColor fallback = %q", f.brandColor())
	}

	if f.appName() != "Laravel" {
...
func TestBrandColor_AppName_DashboardURL_DateFormat_Overrides(t *testing.T) {
	cfg := spark.NewConfigFromValues(map[string]any{
		"spark.brand_color":   "bg-blue",
		"spark.app_name":      "Acme",
		"spark.dashboard_url": "/dash",
		"spark.date_format":   "2006-01-02",
	})
...
func TestGenericTrialEndsAt(t *testing.T) {
	f := NewFrontendState(spark.NewManager(), nil, &fakeSubStore{})

	if f.genericTrialEndsAt(nil) != nil {
		t.Fatalf("nil customer should return nil")
	}

	if f.genericTrialEndsAt(&spark.Customer{}) != nil {
		t.Fatalf("no trial should return nil")
	}

	expired := time.Now().Add(-time.Hour)

	if f.genericTrialEndsAt(&spark.Customer{TrialEndsAt: &expired}) != nil {
		t.Fatalf("expired trial should return nil")
	}

	future := time.Now().Add(48 * time.Hour)

	got := f.genericTrialEndsAt(&spark.Customer{TrialEndsAt: &future})
...
func TestProviderCustomerID(t *testing.T) {
	...
	if providerCustomerID(&spark.Customer{}) != nil {
		t.Fatal("empty PaddleID should return nil")
	}

	if got := providerCustomerID(&spark.Customer{PaddleID: "cus_1"}); got != "cus_1" {
...
func TestSubscriptionState_Populated(t *testing.T) {
	cfg := spark.NewConfigFromValues(map[string]any{"spark.path": "billing"})
	f := NewFrontendState(spark.NewManager(), cfg, &fakeSubStore{})

	plan := &spark.Plan{ID: "pri_a", Name: "Pro"}
	sub := &spark.Subscription{
	...
	if state["status"] != string(spark.StatusAwaitingPayment) {
...
func TestCtaState(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil plan", sub: &spark.Subscription{Status: spark.StatusAwaitingPayment}, plan: nil, visible: false},
		{name: "not awaiting", sub: &spark.Subscription{Status: spark.StatusActive}, plan: plan, visible: false},
		{
		...
		},
		{
			...
			sub:     &spark.Subscription{Status: spark.StatusAwaitingPayment},
			plan:    plan,
			visible: false,
		},
	}
...
func TestPortalURL(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil plan", sub: &spark.Subscription{Status: spark.StatusActive}, plan: nil, want: ""},
		{
			name: "active",
		...
		{
			name: "paused",
			sub:  &spark.Subscription{Status: spark.StatusPaused},
			plan: plan,
			want: "/billing",
		},
		{
		...
		{
			name: "expired status",
			sub:  &spark.Subscription{Status: spark.StatusExpired},
			plan: plan,
			want: "",
		},
	}
...
func TestPortalURL_EmptyPathReturnsRoot(t *testing.T) {
	cfg := spark.NewConfigFromValues(map[string]any{"spark.path": ""})
	f := NewFrontendState(spark.NewManager(), cfg, &fakeSubStore{})

	sub := &spark.Subscription{Status: spark.StatusActive}

	if got := f.portalURL(sub, &spark.Plan{ID: "pri"}); got != "/" {
...
func TestSubscriptionIsActiveOrPastDue(t *testing.T) {
	...
	if !subscriptionIsActiveOrPastDue(&spark.Subscription{Status: spark.StatusActive}) {
		t.Fatal("active should be true")
	}

	if !subscriptionIsActiveOrPastDue(&spark.Subscription{Status: spark.StatusPastDue}) {
		t.Fatal("past due should be true")
	}

	if subscriptionIsActiveOrPastDue(&spark.Subscription{Status: spark.StatusCanceled}) {
		t.Fatal("canceled should be false")
	}
}

func TestInvoiceID(t *testing.T) {
	if got := invoiceID(spark.Transaction{PaddleID: "tx_1", InvoiceNumber: "INV-1"}); got != "tx_1" {
		t.Fatalf("invoiceID prefers PaddleID: %q", got)
	}

	if got := invoiceID(spark.Transaction{InvoiceNumber: "INV-1"}); got != "INV-1" {
...
func TestCurrentAt_HappyPath_ClearsPendingCheckout(t *testing.T) {
	...
	sub := &spark.Subscription{Status: spark.StatusActive, Plan: "pro"}

	customers := &fakeCustomerStore{
		customer: &spark.Customer{PendingCheckoutID: "chk_1", PaddleID: "cus_1"},
	}
...
func TestCurrentAt_PendingState_WhenCustomerHasPendingCheckout(t *testing.T) {
	...
	customers := &fakeCustomerStore{
		customer: &spark.Customer{PendingCheckoutID: "chk_1"},
	}
...
func TestCurrentAt_NoneState_WhenNoSubscription(t *testing.T) {
	mgr := spark.NewManager()
	cfg := spark.NewConfigFromValues(nil)
...
func TestCurrentAt_TransactionsSurfaced(t *testing.T) {
	mgr := spark.NewManager()
	cfg := spark.NewConfigFromValues(map[string]any{"spark.date_format": "2006-01-02"})
...
func TestCurrentAt_PaymentReporterPopulatesPayments(t *testing.T) {
	...
	reporter := &fakePaymentReporter{
		last: &spark.Payment{Amount: 1000, Currency: "USD", Date: time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)},
		next: &spark.Payment{Amount: 2000, Currency: "USD", Date: time.Date(2026, 2, 1, 0, 0, 0, 0, time.UTC)},
	}
...
func TestCurrentAt_PaymentReporterError(t *testing.T) {
	...
	f := NewFrontendState(spark.NewManager(), nil, &fakeSubStore{sub: sub}).WithPaymentReporter(reporter)
...
func TestResolveState_CustomerSaveError(t *testing.T) {
	customers := &fakeCustomerStore{
		customer: &spark.Customer{PendingCheckoutID: "chk"},
		saveErr:  errors.New("boom"),
	}

	f := NewFrontendState(spark.NewManager(), nil, &fakeSubStore{}).WithCustomerStore(customers)
...
func TestResolveState_Canceled(t *testing.T) {
	...
	state, err := f.resolveState(context.Background(), &spark.Subscription{Status: spark.StatusCanceled, EndsAt: &endsAt}, nil)
...
func TestResolveState_PastDue(t *testing.T) {
	...
	state, err := f.resolveState(context.Background(), &spark.Subscription{Status: spark.StatusPastDue}, nil)
...
func TestResolveState_ExpiredFallsToNone(t *testing.T) {
	...
	state, err := f.resolveState(context.Background(), &spark.Subscription{Status: spark.StatusExpired}, nil)
