...
func (e *Access) capacityForSubscription(ctx context.Context, sub *spark.Subscription) int {
	...
	for _, f := range features {
		...
		if spark.SubscriptionFeatureCode(f.Feature.Code) != spark.FeatureActiveRoleCapacity {
			continue
		}
...
func (e *Access) CurrentSubscription(ctx context.Context, billableType string, billableID int64) (*spark.Subscription, error) {
