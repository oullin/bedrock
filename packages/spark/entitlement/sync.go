...
func (s *Synchronizer) Sync(ctx context.Context, sub *spark.Subscription) error {
	return s.txn.RunInTransaction(ctx, func(ctx context.Context) error {
		...
		var syncFeatures []spark.SubscriptionFeature

		for _, pf := range plan.PlanFeatures {
			syncFeatures = append(syncFeatures, spark.SubscriptionFeature{
...
func (s *Synchronizer) SyncActivationState(ctx context.Context, sub *spark.Subscription) error {
	return s.syncActivationState(ctx, sub)
}
