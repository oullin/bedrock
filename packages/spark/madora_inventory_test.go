...
type madoraExpirableStore struct {
	subscriptions []*spark.Subscription
	deleted       []int64
}
...
func (s *madoraSubStore) FindByID(_ context.Context, id int64) (*spark.Subscription, error) {
...
func (s *madoraSubStore) FindByProviderID(_ context.Context, providerID string) (*spark.Subscription, error) {
	for _, sub := range s.subs {
...
func (s *madoraSubStore) CurrentForBillable(_ context.Context, billableType string, billableID int64) (*spark.Subscription, error) {
	for _, sub := range s.subs {
...
func (s *madoraSubStore) ActiveForBillable(_ context.Context, billableType string, billableID int64) ([]*spark.Subscription, error) {
	var matched []*spark.Subscription
...
func (s *madoraSubStore) Create(_ context.Context, sub *spark.Subscription) error {
	s.subs = append(s.subs, sub)

	return nil
}

func (s *madoraSubStore) Save(_ context.Context, sub *spark.Subscription) error {
...
func (s madoraOrderStore) FindByID(context.Context, int64) (*spark.Order, error) { return nil, nil }
func (s madoraOrderStore) FindByBillable(context.Context, int64, int) ([]spark.Order, error) {
	return nil, nil
}
func (s madoraOrderStore) Create(context.Context, *spark.Order) error { return nil }
func (s madoraOrderStore) Save(context.Context, *spark.Order) error   { return nil }
func (s madoraOrderStore) HasCompletedForProduct(context.Context, int64, int64) (bool, error) {
	return false, nil
}

func (s madoraProductStore) FindByID(context.Context, int64) (*spark.Product, error) { return nil, nil }
func (s madoraProductStore) Active(context.Context) ([]spark.Product, error)         { return nil, nil }
func (s madoraProductStore) ActiveSubscriptions(context.Context) ([]spark.Product, error) {
	return nil, nil
}
func (s madoraProductStore) ActiveOneTime(context.Context) ([]spark.Product, error) {
	return nil, nil
}

func (s *madoraTransactionStore) FindByProviderID(_ context.Context, providerID string) (*spark.Transaction, error) {
...
func (s *madoraTransactionStore) Save(_ context.Context, tx *spark.Transaction) error {
...
func (s *madoraExpirableStore) ExpirableSubscriptions(context.Context, time.Time) ([]*spark.Subscription, error) {
...
func testSparkManager() *spark.Manager {
	mgr := spark.NewManager()
	mgr.RegisterBillable(spark.BillableConfig{
		Model:           "team",
		DefaultInterval: "monthly",
	})
	mgr.Billable("team").
		ChargePerSeat("seat", func(b spark.Billable) int {
...
// SparkPortalTest::test_spark_dashboard_url_defaults_to_the_agreement_page
// SparkPortalTest::test_spark_portal_route_keeps_the_default_billing_path
...
// SparkPlanRegistrarTest::test_it_registers_spark_plans_from_the_active_catalog_prices
...
func TestMadoraPlanCatalogAndPortalState(t *testing.T) {
	...
	subscriptions := &madoraSubStore{subs: []*spark.Subscription{{
		ID:           99,
		BillableType: "team",
		BillableID:   10,
		PaddleID:     "sub_123",
...
func TestMadoraFrontendStateSubscriptionStates(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "active", sub: &spark.Subscription{BillableType: "team", BillableID: 10, Status: spark.StatusActive}, want: "active"},
		{name: "trialing", sub: &spark.Subscription{BillableType: "team", BillableID: 10, Status: spark.StatusTrialing, TrialEndsAt: &future}, want: "active"},
		{name: "grace", sub: &spark.Subscription{BillableType: "team", BillableID: 10, Status: spark.StatusCanceled, EndsAt: &future}, want: "onGracePeriod"},
		{name: "inactive", sub: &spark.Subscription{BillableType: "team", BillableID: 10, Status: spark.StatusCanceled, EndsAt: &past}, want: "none"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var subs []*spark.Subscription

			if tt.sub != nil {
				subs = append(subs, tt.sub)
			}

			frontend := state.NewFrontendState(testSparkManager(), spark.NewConfigFromValues(map[string]any{"spark.path": "billing"}), &madoraSubStore{subs: subs})
	...
	store := &madoraSubStore{subs: []*spark.Subscription{
		{BillableType: "team", BillableID: 10, Status: spark.StatusPaused},
		{BillableType: "team", BillableID: 10, Status: spark.StatusPastDue},
	}}
...
func TestMadoraBillingStatePortalAndCTA(t *testing.T) {
	...
	expiresAt := now.AddDate(0, 0, spark.DefaultPendingExpiryDays)
	readyAt := now
	...
	tests := []struct {
		name              string
		sub               *spark.Subscription
		wantStatus        spark.SubscriptionStatus
		wantPlanCode      string
		wantPlanName      string
	...
	}{
		{
			...
			sub: &spark.Subscription{
				...
				Items:            []spark.SubscriptionItem{{PriceID: "pri_pro_yearly"}},
			},
			wantStatus:        spark.StatusAwaitingPayment,
			wantPlanCode:      "pro",
			wantPlanName:      "Pro",
		...
		},
		{
			...
			sub: &spark.Subscription{
				...
				PendingExpiresAt: &expiresAt,
				PaymentReadyAt:   &readyAt,
				Items:            []spark.SubscriptionItem{{PriceID: "pri_missing"}},
			},
			wantStatus:   spark.StatusAwaitingPayment,
			wantPlanCode: "pro",
			wantPlanName: "Pro",
		},
		{
			...
			sub: &spark.Subscription{
				BillableType: "team",
				BillableID:   10,
				Status:       spark.StatusTrialing,
				Plan:         "starter",
				TrialEndsAt:  &trialEndsAt,
			},
			wantStatus:   spark.StatusTrialing,
		...
		},
		{
			...
			sub: &spark.Subscription{
				BillableType: "team",
				BillableID:   10,
				Status:       spark.StatusPaused,
				Plan:         "pro",
				Items:        []spark.SubscriptionItem{{PriceID: "pri_pro_yearly"}},
			},
			wantStatus:    spark.StatusPaused,
			wantPlanCode:  "pro",
			wantPlanName:  "Pro",
			wantPortalURL: "/billing",
		},
		{
			...
			wantStatus:   spark.StatusCanceled,
			wantPlanCode: "pro",
			wantPlanName: "Pro",
		},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			frontend := state.NewFrontendState(
				...
				spark.NewConfigFromValues(map[string]any{
					"spark.path":      "billing",
					"spark.billables": map[string]spark.BillableConfig{"team": {DefaultInterval: "monthly"}},
				}),
				&madoraSubStore{subs: []*spark.Subscription{tt.sub}},
			)
...
func TestMadoraSubscriptionLifecycleStateMachine(t *testing.T) {
	...
	sub := spark.NewPendingSubscription(billable, "pro", now)

	if sub.Status != spark.StatusPending || sub.Plan != "pro" || sub.PendingExpiresAt == nil {
		t.Fatalf("pending subscription = %#v", sub)
	}

	if got := int(sub.PendingExpiresAt.Sub(now).Hours() / 24); got != spark.DefaultPendingExpiryDays {
		t.Fatalf("pending expiry days = %d, want %d", got, spark.DefaultPendingExpiryDays)
	}
	...
	if sub.Status != spark.StatusAwaitingPayment || sub.PaymentReadyAt == nil || !sub.PaymentReadyAt.Equal(now) {
	...
	if !sub.Activate(now.Add(2*time.Hour)) || sub.Status != spark.StatusActive {
		t.Fatalf("active subscription = %#v", sub)
	}

	expired := spark.NewPendingSubscription(billable, "starter", now)

	if !expired.Expire(now.Add(time.Hour)) || expired.Status != spark.StatusExpired {
		t.Fatalf("expired subscription = %#v", expired)
	}

	trial := spark.NewTrialSubscription(billable, "starter", 14, now)

	if trial.Status != spark.StatusTrialing || trial.TrialEndsAt == nil || trial.PaymentReadyAt != nil {
		t.Fatalf("trial subscription = %#v", trial)
	}

	pastDue := &spark.Subscription{Status: spark.StatusActive, UpdatedAt: now}

	if !pastDue.MarkPastDue(now.Add(time.Hour)) || pastDue.Status != spark.StatusPastDue {
	...
	paused := &spark.Subscription{Status: spark.StatusActive, UpdatedAt: now}

	if !paused.Pause(now.Add(time.Hour)) || paused.Status != spark.StatusPaused || paused.PausedAt == nil {
	...
	canceled := &spark.Subscription{Status: spark.StatusActive, UpdatedAt: now}

	if !canceled.Cancel(now.Add(time.Hour)) || canceled.Status != spark.StatusCanceled || canceled.EndsAt == nil {
...
func TestMadoraStaleSubscriptionExpiryBatch(t *testing.T) {
	...
	stalePending := &spark.Subscription{ID: 1, Status: spark.StatusPending, PendingExpiresAt: &past}
	staleAwaiting := &spark.Subscription{ID: 2, Status: spark.StatusAwaitingPayment, PendingExpiresAt: &past}
	freshPending := &spark.Subscription{ID: 3, Status: spark.StatusPending, PendingExpiresAt: &future}
	active := &spark.Subscription{ID: 4, Status: spark.StatusActive, PendingExpiresAt: &past}
	paused := &spark.Subscription{ID: 5, Status: spark.StatusPaused, PendingExpiresAt: &past}
	canceled := &spark.Subscription{ID: 6, Status: spark.StatusCanceled, PendingExpiresAt: &past}
	staleTrial := &spark.Subscription{ID: 7, Status: spark.StatusTrialing, Plan: "starter", PendingExpiresAt: &past}

	store := &madoraExpirableStore{subscriptions: []*spark.Subscription{
		stalePending,
		staleAwaiting,
		freshPending,
		active,
	...
	expired, err := spark.ExpireStaleSubscriptions(context.Background(), store, now)
	...
	if stalePending.Status != spark.StatusExpired || staleAwaiting.Status != spark.StatusExpired || staleTrial.Status != spark.StatusExpired {
		t.Fatalf("stale statuses = %s/%s/%s, want expired", stalePending.Status, staleAwaiting.Status, staleTrial.Status)
	}

	if freshPending.Status != spark.StatusPending || active.Status != spark.StatusActive ||
		paused.Status != spark.StatusPaused || canceled.Status != spark.StatusCanceled {
...
func TestMadoraSubscriptionAccessTransitions(t *testing.T) {
	if !spark.StatusActive.GrantsAccess() || !spark.StatusPastDue.GrantsAccess() || !spark.StatusTrialing.GrantsAccess() {
		t.Fatalf("active, past_due, and trialing statuses should grant access")
	}

	if spark.StatusPaused.GrantsAccess() || spark.StatusCanceled.GrantsAccess() {
		t.Fatalf("paused and canceled statuses should not grant access")
	}

	sub := &spark.Subscription{BillableType: "team", BillableID: 10, Status: spark.StatusPastDue, PaddleID: "sub_123"}
	store := &madoraSubStore{subs: []*spark.Subscription{sub}}
	billing := service.NewBillingService(store, madoraOrderStore{}, madoraProductStore{})
	...
	if sub.Status != spark.StatusCanceled || sub.EndsAt == nil || len(store.saved) != 1 {
	...
	if sub.Status != spark.StatusActive || sub.EndsAt != nil {
...
func TestMadoraCheckoutCreationIsStable(t *testing.T) {
	...
	plan := spark.NewPlan("Pro", "pri_pro_monthly")
...
func TestMadoraCanonicalMoneySchema(t *testing.T) {
	if spark.DefaultCurrency != "USD" || !spark.IsSupportedCurrency("usd") {
		t.Fatalf("default currency = %s, supported USD = %v", spark.DefaultCurrency, spark.IsSupportedCurrency("usd"))
	}

	total, err := spark.ParseMinorAmount("12345")

	if err != nil || total != 12345 {
		t.Fatalf("ParseMinorAmount string = %d, %v; want 12345", total, err)
	}

	signed, err := spark.ParseMinorAmount("-42")

	if err != nil || signed != -42 {
		t.Fatalf("ParseMinorAmount signed string = %d, %v; want -42", signed, err)
	}

	if _, err := spark.ParseMinorAmount("12xyz"); err == nil {
		t.Fatalf("malformed minor amount string accepted")
	}

	if err := spark.ValidateTransactionMoney(12345, 123, "USD"); err != nil {
		t.Fatalf("valid transaction money rejected: %v", err)
	}

	if err := spark.ValidateTransactionMoney(0, 0, "USD"); err != nil {
		t.Fatalf("zero transaction money rejected: %v", err)
	}

	if err := spark.ValidateTransactionMoney(-1, 0, "USD"); err == nil {
		t.Fatalf("negative total accepted")
	}

	if err := spark.ValidateTransactionMoney(0, -1, "USD"); err == nil {
		t.Fatalf("negative tax accepted")
	}

	if err := spark.ValidateTransactionMoney(0, 0, "ZZZ"); err == nil {
		t.Fatalf("unknown currency accepted")
	}

	if err := spark.ValidatePriceMoney(0, "USD"); err != nil {
		t.Fatalf("zero price money rejected: %v", err)
	}

	if err := spark.ValidatePriceMoney(-1, "USD"); err == nil {
		t.Fatalf("negative price money accepted")
	}

	tx := spark.Transaction{Total: total, Tax: 123, Currency: "USD"}
...
func TestMadoraPlanPriceRotation(t *testing.T) {
	...
	history := []spark.PlanPeriodPrice{{
		ID:              1,
		AmountMinor:     &amount,
		Currency:        "USD",
		PricingMode:     spark.PlanPricingModeMoney,
		ProviderPriceID: "pri_existing",
		Active:          true,
	}}

	if _, ok := reflect.TypeOf(spark.PlanPeriodPrice{}).FieldByName("Amount"); ok {
		t.Fatalf("PlanPeriodPrice should not expose legacy Amount storage")
	}

	rotated, err := spark.RotatePlanPrice(history, spark.PlanPeriodPrice{
		ID:              2,
	...
	if _, err := spark.RotatePlanPrice(rotated, spark.PlanPeriodPrice{
		AmountMinor:     &rotatedAmount,
		Currency:        "USD",
		PricingMode:     spark.PlanPricingModeMoney,
		ProviderPriceID: " pri_rotated ",
	...
	free, err := spark.RotatePlanPrice(nil, spark.PlanPeriodPrice{PricingMode: spark.PlanPricingModeFree})

	if err != nil || free[0].DisplayAmount() != "Free" || free[0].AmountMinor != nil || free[0].Currency != "" {
		t.Fatalf("free price = %#v, err = %v", free, err)
	}

	custom, err := spark.RotatePlanPrice(free, spark.PlanPeriodPrice{PricingMode: spark.PlanPricingModeCustom})
	...
	inactiveHistory := []spark.PlanPeriodPrice{
		{ID: 1, ProviderPriceID: "pri_old_one", PricingMode: spark.PlanPricingModeMoney, AmountMinor: &amount, Currency: "USD", Active: false},
		{ID: 2, ProviderPriceID: "pri_old_two", PricingMode: spark.PlanPricingModeMoney, AmountMinor: &amount, Currency: "USD", Active: false},
	}
	withInactive, err := spark.RotatePlanPrice(inactiveHistory, spark.PlanPeriodPrice{
		AmountMinor:     &rotatedAmount,
		Currency:        "USD",
		PricingMode:     spark.PlanPricingModeMoney,
...
func TestMadoraWebhookSubscriptionUpdateSyncsLocalState(t *testing.T) {
	sub := &spark.Subscription{
		...
		BillableID:   10,
		PaddleID:     "sub_123",
		Status:       spark.StatusPastDue,
		Items:        []spark.SubscriptionItem{{PriceID: "pri_old", Quantity: 2}},
	}
	subs := &madoraSubStore{subs: []*spark.Subscription{sub}}
	events := &madoraDispatcher{}
	handler := webhook.NewHandler(subs, nil, &madoraTransactionStore{transactions: map[string]*spark.Transaction{}}, events)
	...
	if !containsEvent[spark.WebhookReceivedEvent](events.events) ||
		!containsEvent[spark.SubscriptionUpdatedEvent](events.events) ||
		!containsEvent[spark.WebhookHandledEvent](events.events) {
...
func TestMadoraReconcileSubscriptionAfterCheckout(t *testing.T) {
	pending := &spark.Subscription{
		...
		BillableType: "team",
		BillableID:   42,
		Type:         spark.DefaultSubscriptionType,
		Plan:         "pro",
		Status:       spark.StatusAwaitingPayment,
	...
	if cashier.Plan != "pro" || cashier.Type != spark.DefaultSubscriptionType {
	...
	if !containsEvent[spark.SubscriptionUpdatedEvent](events.events) {
		t.Fatalf("events = %#v, want subscription update", events.events)
	}

	noPendingStore := &madoraSubStore{subs: []*spark.Subscription{cashier}}
	reconciled, err = spark.ReconcileSubscriptionAfterCheckout(context.Background(), noPendingStore, "team", 42, cashier, events)
...
func TestMadoraWebhookTransactionCompletedPersistsTransaction(t *testing.T) {
	transactions := &madoraTransactionStore{transactions: map[string]*spark.Transaction{}}
	events := &madoraDispatcher{}
	...
	if tx.PaddleID != "txn_123" || tx.Status != spark.TransactionCompleted || tx.Total != 2500 || tx.Tax != 250 || tx.Currency != "USD" {
		t.Fatalf("transaction = %#v", tx)
	}

	if !containsEvent[spark.TransactionCompletedEvent](events.events) {
		t.Fatalf("events = %#v", events.events)
	}
}
...
func TestMadoraWebhookTransactionRejectsInvalidMoneyPayloads(t *testing.T) {
	...
	existing := &spark.Transaction{PaddleID: "txn_update_invalid", Status: spark.TransactionReady, Total: 2500, Tax: 250, Currency: "SGD"}
	transactions := &madoraTransactionStore{transactions: map[string]*spark.Transaction{"txn_update_invalid": existing}}
	events := &madoraDispatcher{}
	handler := webhook.NewHandler(&madoraSubStore{}, nil, transactions, events)
	payload := map[string]any{
	...
	if existing.Status != spark.TransactionReady || existing.Total != 2500 || existing.Tax != 250 || existing.Currency != "SGD" {
...
func TestMadoraWebhookTransactionUpdatedRefreshesTransactionMoney(t *testing.T) {
	existing := &spark.Transaction{PaddleID: "txn_123", Status: spark.TransactionReady, Total: 2500, Tax: 250, Currency: "USD"}
	transactions := &madoraTransactionStore{transactions: map[string]*spark.Transaction{"txn_123": existing}}
	events := &madoraDispatcher{}
	handler := webhook.NewHandler(&madoraSubStore{}, nil, transactions, events)
	...
	if existing.Status != spark.TransactionCompleted || existing.Total != 3000 || existing.Tax != 0 || existing.Currency != "USD" {
