...
func NewPortalHandler(manager *spark.Manager, frontendState *subscription.FrontendStateBuilder) *PortalHandler {
...
func (h *PortalHandler) Show(w http.ResponseWriter, r *http.Request) {
	...
	if !h.manager.IsAuthorized(billable, r) {
		http.Error(w, spark.ErrUnauthorized.Error(), http.StatusForbidden)
