...
func NewInquiryHandler(mailer spark.Mailer, config *spark.Config) *InquiryHandler {
...
func (h *InquiryHandler) Store(w http.ResponseWriter, r *http.Request) {
	...
	if contactEmail != "" {
		_ = h.mailer.Send(ctx, contactEmail, &spark.CustomPlanInquiryMail{
			Name:    input.Name,
			Email:   input.Email,
	...
	_ = h.mailer.Send(ctx, input.Email, &spark.CustomPlanInquiryConfirmationMail{Name: input.Name})
...
func (c *InquiryInput) Validate() spark.ValidationErrors {
	var errs spark.ValidationErrors

	if strings.TrimSpace(c.Name) == "" {
		errs = append(errs, spark.ValidationError{Field: "name", Message: "Your name is required."})
	} else if len(c.Name) > 150 {
		errs = append(errs, spark.ValidationError{Field: "name", Message: "Your name must not be greater than 150 characters."})
	}

	if strings.TrimSpace(c.Email) == "" {
		errs = append(errs, spark.ValidationError{Field: "email", Message: "An email address is required."})
	} else if len(c.Email) > 255 || !strings.Contains(c.Email, "@") {
		errs = append(errs, spark.ValidationError{Field: "email", Message: "Please provide a valid email address."})
	}

	if len(c.Company) > 150 {
		errs = append(errs, spark.ValidationError{Field: "company", Message: "Company name must not be greater than 150 characters."})
	}

	if strings.TrimSpace(c.Message) == "" {
		errs = append(errs, spark.ValidationError{Field: "message", Message: "A message is required."})
	} else if len(c.Message) > 2000 {
		errs = append(errs, spark.ValidationError{Field: "message", Message: "A message must not be greater than 2000 characters."})
	}
