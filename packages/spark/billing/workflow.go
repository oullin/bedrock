...
func (w *Workflow) ReadBillingState(ctx context.Context, billableType string, billableID int64) spark.BillingStateSnapshot {
	return w.stateRes.Read(ctx, billableType, billableID)
}

// StartPendingSubscription creates a pending subscription.
func (w *Workflow) StartPendingSubscription(ctx context.Context, billable spark.Billable, plan spark.SubscriptionPlan, period spark.BillingPeriod) (*spark.Subscription, error) {
	return w.subs.StartPendingSubscription(ctx, billable, plan, period)
}

// StartStarterTrial creates a starter trial subscription.
func (w *Workflow) StartStarterTrial(ctx context.Context, billable spark.Billable) (*spark.Subscription, error) {
	return w.subs.StartStarterTrial(ctx, billable)
}

// StartCheckout initiates a checkout for the plan and period.
func (w *Workflow) StartCheckout(ctx context.Context, billable spark.Billable, plan spark.SubscriptionPlan, period spark.BillingPeriod) (string, error) {
	return w.starter.Start(ctx, billable, plan, period)
}

// MarkPaymentReady transitions a subscription to awaiting payment.
func (w *Workflow) MarkPaymentReady(ctx context.Context, sub *spark.Subscription) (*spark.Subscription, error) {
	return w.lifecycle.MarkPaymentReady(ctx, sub)
}

// Activate transitions a subscription to active.
func (w *Workflow) Activate(ctx context.Context, sub *spark.Subscription) (*spark.Subscription, error) {
	return w.lifecycle.Activate(ctx, sub)
}

// MarkPastDue transitions a subscription to past due.
func (w *Workflow) MarkPastDue(ctx context.Context, sub *spark.Subscription) (*spark.Subscription, error) {
	return w.lifecycle.MarkPastDue(ctx, sub)
}

// Pause transitions a subscription to paused.
func (w *Workflow) Pause(ctx context.Context, sub *spark.Subscription) (*spark.Subscription, error) {
	return w.lifecycle.Pause(ctx, sub)
}

// Cancel transitions a subscription to canceled.
func (w *Workflow) Cancel(ctx context.Context, sub *spark.Subscription) (*spark.Subscription, error) {
	return w.lifecycle.Cancel(ctx, sub)
}

// Expire transitions a subscription to expired.
func (w *Workflow) Expire(ctx context.Context, sub *spark.Subscription) (*spark.Subscription, error) {
...
func (w *Workflow) CurrentSubscription(ctx context.Context, billableType string, billableID int64) (*spark.Subscription, error) {
	return w.subs.CurrentSubscription(ctx, billableType, billableID)
}

// ReconcileCreated handles post-checkout reconciliation.
func (w *Workflow) ReconcileCreated(ctx context.Context, sub *spark.Subscription, billableID int64) error {
	return w.reconciler.ReconcileCreated(ctx, sub, billableID)
}

// SyncUpdated syncs a subscription after a provider update event.
func (w *Workflow) SyncUpdated(ctx context.Context, sub *spark.Subscription) error {
...
func (w *Workflow) SyncEntitlements(ctx context.Context, sub *spark.Subscription) error {
