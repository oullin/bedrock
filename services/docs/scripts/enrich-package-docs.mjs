...
const docsInventoryPath = path.join(root, 'services/compliance/docs-inventories/upstream-docs.txt')
const validUpstreamSources = new Set(read(docsInventoryPath).split('\n').filter((line) => line && !line.startsWith('#')))
...
const laravelSources = {
  ...
  console: ['cli.md#introduction', 'cli.md#defining-input-expectations', 'scheduling.md#task-scheduling'],
  ...
  database: ['database.md#database', 'database.md#introduction', 'queries.md#queries', 'queries.md#running-database-queries', 'migrations.md#migrations', 'migrations.md#introduction', 'orm.md#orm', 'orm.md#introduction', 'seeding.md#seeding', 'seeding.md#introduction'],
  broadcastclient: ['broadcasting.md#client-side-installation', 'websockets.md#introduction'],
  encryption: ['encryption.md#introduction', 'encryption.md#using-the-encrypter'],
  remotetasks: ['remotetasks.md#introduction', 'remotetasks.md#writing-tasks', 'remotetasks.md#notifications'],
  ...
  authflows: ['authflows.md#introduction', 'authflows.md#authentication', 'authflows.md#registration', 'authflows.md#password-reset'],
  hashing: ['hashing.md#introduction', 'hashing.md#basic-usage', 'hashing.md#determining-if-a-password-needs-to-be-rehashed'],
  jobqueue: ['jobqueue.md#introduction', 'jobqueue.md#configuration', 'jobqueue.md#monitoring', 'jobqueue.md#balancing-strategies'],
  httpx: ['requests.md#input', 'responses.md#creating-responses', 'http-client.md#introduction', 'http-tests.md#making-requests'],
  inception: ['authflows.md#introduction', 'tokens.md#introduction', 'oauthserver.md#introduction'],
  inertia: ['frontend.md#using-react-svelte-or-vue', 'responses.md#redirects', 'csrf.md#csrf-protection'],
  authkit: ['authflows.md#introduction', 'tokens.md#spa-authentication', 'tokens.md#api-token-authentication'],
  ...
  logtail: ['logtail.md#introduction', 'logging.md#writing-log-messages'],
  oauthserver: ['oauthserver.md#introduction', 'oauthserver.md#password-grant', 'oauthserver.md#personal-access-tokens', 'oauthserver.md#protecting-routes'],
  featureflags: ['featureflags.md#introduction', 'featureflags.md#defining-features', 'featureflags.md#scope', 'featureflags.md#adding-custom-featureflags-drivers'],
  pipeline: ['helpers.md#other-utilities', 'middleware.md#middleware'],
  httppreview: ['httppreview.md#introduction', 'httppreview.md#handling-file-uploads', 'httppreview.md#managing-side-effects'],
  ...
  websockets: ['websockets.md#introduction', 'websockets.md#application-credentials', 'websockets.md#running-websockets-in-production', 'broadcasting.md#client-side-installation'],
  ...
  search: ['search.md#introduction', 'search.md#searchable-models', 'search.md#querying', 'search.md#search'],
  ...
  socialauth: ['socialauth.md#introduction', 'socialauth.md#routing', 'socialauth.md#authentication-and-storage'],
  billing: ['billing.md#introduction', 'cashier.md#subscriptions'],
  ...
  debugbar: ['debugbar.md#introduction', 'debugbar.md#watchers', 'debugbar.md#dashboard-authorization'],
  ...
  routegen: ['urls.md#urls-for-controller-actions', 'routing.md#named-routes', 'frontend.md#using-react-svelte-or-vue'],
}
...
function testInventory(sourceRoot) {
  return walk(sourceRoot, (file) => /(_test\.go)$/.test(file))
    .filter((file) => /inventory|upstream|compliance|parity/i.test(path.basename(file)))
...
function detectCapabilities(sourceRoot, symbols) {
  ...
  const checks = [
    ...
    ['Database-backed persistence', /\b(database|migration|query|schema|orm|sql|sqlite|mysql|postgres)\b/],
...
function existingUpstreamComments(existing, slug) {
  const existingSources = (existing.match(/^<!-- upstream-docs:[\s\S]*?-->$/gm) ?? [])
    .map((comment) => comment.replace(/^<!--\s*upstream-docs:\s*/, '').replace(/\s*-->$/, ''))
  const preferred = laravelSources[slug] ?? existingSources
  const sources = preferred.filter((source) => validUpstreamSources.has(source))

  if (sources.length === 0) {
    const fallback = `${slug}.md#introduction`
    if (validUpstreamSources.has(fallback)) sources.push(fallback)
  }

  return sources.map((source) => `<!-- upstream-docs: ${source} -->`).join('\n')
}
...
function guideFor(slug) {
  ...
  const comments = existingUpstreamComments(existing, slug)
  ...
  const summary = firstParagraph(rootDoc, `The ${titleFor(slug)} package provides Bedrock's Go implementation for this Upstream-aligned surface.`)
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under \`packages/${slug}\` cover the supported creation paths, default values, and Upstream parity behavior.
