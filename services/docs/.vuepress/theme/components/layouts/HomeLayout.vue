...
<template>
  <div class="flex min-h-full flex-col bg-[#0f0f12] antialiased text-white">
      ...
      <div class="relative mx-auto grid max-w-6xl grid-cols-1 gap-0 md:grid-cols-2 md:divide-x md:divide-white/10">
        ...
        <div class="flex flex-col justify-center gap-8 p-10 md:py-20 md:pr-16">
          ...
          <a
            ...
            class="w-fit font-mono text-xs uppercase tracking-widest text-white/50 hover:text-white/80 transition-colors"
          >
            Open Source · Go · MIT
          </a>
          ...
          <div v-if="frontmatter.actions?.length" class="flex flex-wrap gap-3">
            ...
            >
              {{ action.text }}
              <svg v-if="action.type === 'primary'" class="h-3.5 w-3.5" fill="none" viewBox="0 0 12 12" aria-hidden="true">
                <path d="M3.5 9L7 5.5 3.5 2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </RouteLink>
          </div>
        </div>
        ...
        <div class="flex flex-col justify-center p-10 md:py-20 md:pl-16">
          <!-- Tab bar -->
          <div class="mb-3 flex gap-1 border-b border-white/10 pb-3">
            <button
          ...
          <!-- Command block -->
          <div class="group relative rounded-xl bg-white/5 ring-1 ring-white/10 p-5">
            <pre class="overflow-x-auto font-mono text-sm leading-relaxed"><span class="text-white/40 select-none">$ </span><span class="text-emerald-400">{{ managers[activeTab].cmd }}</span></pre>
            <!-- Copy button -->
            <button
              type="button"
              class="absolute right-3 top-3 rounded p-1.5 text-white/30 opacity-0 group-hover:opacity-100
                     hover:bg-white/10 hover:text-white transition-all"
              aria-label="Copy"
              @click="() => navigator.clipboard.writeText(managers[activeTab].cmd)"
            >
    ...
    <section v-if="frontmatter.features?.length" class="mx-auto w-full max-w-6xl px-10 py-20">
      ...
      <div class="grid grid-cols-1 gap-px bg-white/10 sm:grid-cols-2 xl:grid-cols-3 rounded-2xl overflow-hidden">
        <RouteLink
          v-for="feature in frontmatter.features"
          :key="feature.title"
          :to="`/packages/${feature.title}`"
          class="group relative flex flex-col bg-[#0f0f12] p-8 hover:bg-white/[0.03] transition-colors cursor-pointer"
        >
          ...
          <div class="flex items-start justify-between mb-4">
            ...
            <svg
              ...
              fill="none" viewBox="0 0 12 12" aria-hidden="true"
            >
              <path d="M3.5 9L7 5.5 3.5 2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
          </div>
    ...
    <section class="border-t border-white/10 bg-[#0c0c0f]">
      <div class="mx-auto max-w-6xl px-10 py-20 text-center">
        ...
        <RouteLink
          to="/getting-started"
          class="mt-8 inline-flex items-center gap-2 rounded-lg bg-white px-6 py-3
                 text-sm font-semibold text-zinc-900 shadow-sm hover:bg-zinc-100 transition-colors"
        >
          Get started
          <svg class="h-4 w-4" fill="none" viewBox="0 0 12 12" aria-hidden="true">
            <path d="M3.5 9L7 5.5 3.5 2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </RouteLink>
      </div>
