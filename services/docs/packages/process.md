...
The `process` package provides a Go process runner inspired by Laravel's
...
result, err := runner.
    Command(process.Shell("php artisan queue:work")).
...
```go
runner := process.New().
    Fake("php artisan *", process.NewResult(process.Command{}, 0, "done", "")).
    PreventStrayProcesses()

_, _ = runner.Run(ctx, process.Shell("php artisan migrate"))
_ = runner.AssertRan(process.Shell("php artisan migrate"))
```
...
poolResults, err := runner.Pool().
    Command("first", process.Shell("echo one")).
    Command("second", process.Shell("echo two")).
    Run(ctx)
