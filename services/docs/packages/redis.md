...
| Package         | Purpose                                                           |
| --------------- | ----------------------------------------------------------------- |
| `redis`         | Package redis is a Go port of Upstream's Framework/Redis package. |
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/redis` cover the supported creation paths, default values, and Upstream parity behavior.
