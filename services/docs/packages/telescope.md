...
```bash
GOWORK=./storage/.cache/go.work go test -count=1 ./packages/telescope/...
```
...
| Package | Purpose |
| --- | --- |
| `telescope` | Package telescope provides a debugging and introspection tool for Go applications, mirroring Laravel Telescope. It captures HTTP requests, database queries, exceptions, log messages, events, queued jobs, cache operations, mail, notifications, model changes, views, commands, scheduled tasks, Redis commands, authorization gates, outbound HTTP client calls, and debug dumps. Every captured item is a typed, UUID-keyed entry grouped into batches and persisted via a pluggable repository contract. |
...
The telescope reference is organized around the exported Go surface for package `telescope`. Start from the source coverage and public surface tables to identify the constructors, managers, interfaces, sentinel errors, and helper functions available to callers. Use the package tests as executable wiring examples for collaborators, default behavior, and Laravel parity expectations.
...
import (
    _ "github.com/bedrock/packages/telescope"
)
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/telescope` cover the supported creation paths, default values, and Laravel parity behavior.
...
- `packages/telescope/compliance_inventory_test.go`
...
| `Telescope` | Source-backed public surface. See the Go package for exact signature and behavior. |
| `TelescopeServiceProvider` | Source-backed public surface. See the Go package for exact signature and behavior. |
| `TelescopeTestCase` | Source-backed public surface. See the Go package for exact signature and behavior. |
...
| `NewTelescopeServiceProvider` | Source-backed public surface. See the Go package for exact signature and behavior. |
...
| `SetTelescope` | Source-backed public surface. See the Go package for exact signature and behavior. |
