...
```bash
GOWORK=/Users/gocanto/Sites/bedrock/storage/.cache/go.work go test -count=1 ./packages/jetstream/...
```
...
| Package     | Purpose                                                                                                                                       |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `jetstream` | Package jetstream holds Laravel Jetstream parity tests for Bedrock's Inception-backed team, profile, browser-session, and API-token behavior. |
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/jetstream` cover the supported creation paths, default values, and Laravel parity behavior.
