...
<!-- laravel-docs: events.md#introduction -->
<!-- laravel-docs: events.md#defining-events -->
...
```bash
GOWORK=/Users/gocanto/Sites/bedrock/storage/.cache/go.work go test -count=1 ./packages/events/...
```
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/events` cover the supported creation paths, default values, and Laravel parity behavior.
