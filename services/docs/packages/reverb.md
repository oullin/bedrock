...
| Package  | Purpose                                                                     |
| -------- | --------------------------------------------------------------------------- |
| `reverb` | Package reverb implements a Go port of the upstream Reverb WebSocket server. |
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/reverb` cover the supported creation paths, default values, and parity behavior.
