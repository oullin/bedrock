...
```bash
GOWORK=/Users/gocanto/Sites/bedrock/storage/.cache/go.work go test -count=1 ./packages/websockets/...
```
...
import (
    _ "github.com/bedrock/packages/websockets"
)
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/websockets` cover the supported creation paths, default values, and Upstream parity behavior.
