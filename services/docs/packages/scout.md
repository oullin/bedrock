...
Package scout provides full-text search with pluggable engine backends. It is the Go port of Scout package, adapted to Go idioms while maintaining 100% function parity.
...
```bash
GOWORK=./storage/.cache/go.work go test -count=1 ./packages/scout/...
```
...
The scout reference is organized around the exported Go surface for package `scout`. Start from the source coverage and public surface tables to identify the constructors, managers, interfaces, sentinel errors, and helper functions available to callers. Use the package tests as executable wiring examples for collaborators, default behavior.
...
import (
    _ "github.com/bedrock/packages/scout"
)
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/scout` cover the supported creation paths, default values, and parity behavior.
...
- `packages/scout/builder_inventory_test.go`
- `packages/scout/engines/collection_inventory_test.go`
- `packages/scout/engines/database_inventory_test.go`
