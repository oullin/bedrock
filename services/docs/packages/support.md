...
General-purpose helpers and types — a Go port of `Illuminate\Support`.
...
`Fluent` is a dynamic key-value bag inspired by Laravel's `Fluent` class:
...
`Optional[T]` wraps a nullable value — a Go equivalent of Laravel's `optional()`:
