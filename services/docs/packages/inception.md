...
team-management scaffold. It combines Upstream AuthFlows (login, registration,
password reset, 2FA, email verification) and AuthKit (teams, API tokens,
profile management) into a single Go module that depends only on shared
contracts.

Unlike `auth`, `authflows`, and `authkit` — which are composable primitives —
...
The `RoleRegistry` exposes AuthKit-style per-team roles:
...
## When to Use `inception` vs `auth` + `authflows` + `authkit`
