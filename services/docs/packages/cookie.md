...
```bash
GOWORK=/Users/gocanto/Sites/bedrock/storage/.cache/go.work go test -count=1 ./packages/cookie/...
```
...
| Package  | Purpose                                                                                                                                                                                                                                                |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `cookie` | Package cookie provides Upstream-inspired cookie management primitives. It includes a queuing cookie jar, factory interfaces, and HTTP middleware for transparent cookie encryption/decryption and automatic attachment of queued cookies to responses. |
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/cookie` cover the supported creation paths, default values, and Upstream parity behavior.
