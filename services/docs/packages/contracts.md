...
The contracts package provides Bedrock's Go implementation for this Upstream-aligned surface.
...
```bash
GOWORK=/Users/gocanto/Sites/bedrock/storage/.cache/go.work go test -count=1 ./packages/contracts/...
```
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/contracts` cover the supported creation paths, default values, and Upstream parity behavior.
