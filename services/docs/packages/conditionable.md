...
```bash
GOWORK=/Users/gocanto/Sites/bedrock/storage/.cache/go.work go test -count=1 ./packages/conditionable/...
```
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/conditionable` cover the supported creation paths, default values, and Laravel parity behavior.
