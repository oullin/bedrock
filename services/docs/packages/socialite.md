...
Package socialite provides OAuth1 and OAuth2 social authentication, mirroring Socialite in idiomatic Go.
...
```bash
GOWORK=./storage/.cache/go.work go test -count=1 ./packages/socialite/...
```
...
| Package     | Purpose                                                                                                  |
| ----------- | -------------------------------------------------------------------------------------------------------- |
| `socialite` | Package socialite provides OAuth1 and OAuth2 social authentication, mirroring Socialite in idiomatic Go. |

## Core Concepts

The socialite reference is organized around the exported Go surface for package `socialite`. Start from the source coverage and public surface tables to identify the constructors, managers, interfaces, sentinel errors, and helper functions available to callers. Use the package tests as executable wiring examples for collaborators, default behavior.

### Public Surface

| Surface                    | Exported API                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Types                      | `AbstractProvider`, `BitbucketProvider`, `DriverFactory`, `FacebookProvider`, `FakeProvider`, `GithubProvider`, `GitlabProvider`, `GoogleProvider`, `HTTPDoer`, `LinkedInProvider`, `Manager`, `OAuth1Server`, `OneAbstractProvider`, `Provider`, `ProviderConfig`, `ProviderHooks`, `Session`, `SlackProvider`, `SocialiteServiceProvider`, `TemporaryCredentials`, and 4 more |
...
| Variables                  | `ErrInvalidState`, `ErrMissingTemporaryCredentials`, `ErrMissingVerifier`, `Socialite`                                                                                                                                                                                                                                                                                          |
...
import (
    _ "github.com/bedrock/packages/socialite"
)
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/socialite` cover the supported creation paths, default values, and parity behavior.
...
| `SocialiteServiceProvider` | Source-backed public surface. See the Go package for exact signature and behavior. |
...
| `NewSocialiteServiceProvider` | Source-backed public surface. See the Go package for exact signature and behavior. |
...
| `Socialite`                      | Source-backed public surface. See the Go package for exact signature and behavior. |
