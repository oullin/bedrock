...
```bash
GOWORK=/Users/gocanto/Sites/bedrock/storage/.cache/go.work go test -count=1 ./packages/socialauth/...
```
...
| Package     | Purpose                                                                                                          |
| ----------- | ---------------------------------------------------------------------------------------------------------------- |
| `socialauth` | Package socialauth provides OAuth1 and OAuth2 social authentication, mirroring Upstream\SocialAuth in idiomatic Go. |
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/socialauth` cover the supported creation paths, default values, and Upstream parity behavior.
