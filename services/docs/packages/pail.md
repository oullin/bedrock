...
```bash
GOWORK=./storage/.cache/go.work go test -count=1 ./packages/pail/...
```
...
| Package | Purpose                                                                               |
| ------- | ------------------------------------------------------------------------------------- |
| `pail`  | Package pail provides log tailing and filtering primitives inspired by upstream Pail. |

## Core Concepts

The pail reference is organized around the exported Go surface for package `pail`. Start from the source coverage and public surface tables to identify the constructors, managers, interfaces, sentinel errors, and helper functions available to callers. Use the package tests as executable wiring examples for collaborators, default behavior.
...
import (
    _ "github.com/bedrock/packages/pail"
)
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/pail` cover the supported creation paths, default values, and parity behavior.
