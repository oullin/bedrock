...
<!-- upstream-docs: oauthserver.md#password-grant -->
...
```bash
GOWORK=/Users/gocanto/Sites/bedrock/storage/.cache/go.work go test -count=1 ./packages/oauthserver/...
```
...
import (
    _ "github.com/bedrock/packages/oauthserver"
)
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/oauthserver` cover the supported creation paths, default values, and Upstream parity behavior.
