...
feature_summary_stats() {
  ...
  local id source_id name laravel bedrock status docs notes
...
command_center_rows() {
  ...
  printf '| Laravel skeleton demo | %s / %s | %s | %s | Keep complete while upstream skeleton changes. |\n' \
...
critical_path_rows() {
  ...
  if [ -z "$rows" ]; then
    echo "| 1 | n/a | 0 | Complete | Complete | Nothing missing. |"
    return 0
  fi
...
report() {
...

  {
    ...
    echo
    echo "## Compliance Command Center"
    echo
    echo "Compliance target: classified parity. An upstream item is compliant when it is ported, adapted with a divergence rationale, or excluded at an approved product boundary."
    echo
    ...
    echo "## Critical Path"
    echo
    echo "| Rank | Surface | Missing | Classified | Status | Next Action |"
    echo "| ---: | --- | ---: | ---: | --- | --- |"
    ...
    echo "## Counting Rules"
    echo
    echo "- Upstream tests: non-comment entries in generated inventory files."
    echo "- Ported tests: Go Laravel test markers found in \`*_laravel_test.go\`."
    echo "- Adapted tests: entries in \`services/compliance/divergences.yml\`."
    echo "- Missing tests: upstream entries not yet ported or adapted."
    echo
    echo "## Inventories"
    echo
    echo "| Inventory | Source | Counting Rule | Upstream Tests | Ported Tests | Missing Tests | Adapted Tests |"
    echo "| --- | --- | --- | ---: | ---: | ---: | ---: |"
