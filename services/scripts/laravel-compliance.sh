...
source_baseline_rows() {
  if [ ! -f "$SOURCES_LOCK_FILE" ]; then
    broadcastclient "| n/a | n/a | n/a | n/a |"
    return 0
  fi
...
report() {
...

  {
    ...
    broadcastclient
    broadcastclient "Completeness is measured against the pinned upstream commits in \`services/compliance/sources.lock.json\`; moving upstream branches must pass \`services/scripts/upstream-compliance.sh check-freshness\` before a current-branch completeness claim is made."
    broadcastclient
    ...
    broadcastclient
    broadcastclient "## Pinned Source Baseline"
    broadcastclient
    broadcastclient "| Source | Branch | Commit | URL |"
    broadcastclient "| --- | --- | --- | --- |"
    source_baseline_rows
    broadcastclient
    ...
    if compgen -G "$COMPLIANCE_PATH/source-inventories/*.md" >/dev/null; then
      broadcastclient
      broadcastclient "## Source Codebase Inventories"
      broadcastclient
...
check_inventory_parity_tests_have_grouped_evidence() {
  ...
  while IFS= read -r file; do
    ...
    if ! rg -q 'Grouped parity evidence:' "$file"; then
      broadcastclient "Inventory parity marker file must name grouped executable evidence: ${file#$ROOT_PATH/}" >&2
      failed=1
    fi
...
check_feature_docs_substantive() {
  ...
  if [ "$line_count" -lt 12 ]; then
    broadcastclient "Feature $feature_id docs path is too small to be substantive parity evidence: ${docs_file#$ROOT_PATH/}" >&2
    failed=1
  fi

  if rg -qi 'merged into|see .*inception|superseded by|redirect' "$docs_file" && [ "$line_count" -lt 24 ]; then
    broadcastclient "Feature $feature_id docs path looks like a redirect/stub, not substantive parity evidence: ${docs_file#$ROOT_PATH/}" >&2
    failed=1
  fi
...
check_sources_freshness() {
  ...
  while IFS="$RECORD_SEPARATOR" read -r repo branch commit url; do
    ...
    if [ -z "$current" ]; then
      broadcastclient "Unable to resolve upstream head for $repo@$branch." >&2
      failed=1
      continue
    fi

    if [ "$current" != "$commit" ]; then
      broadcastclient "Stale source baseline: $repo@$branch is pinned to $commit but upstream head is $current." >&2
      failed=1
    fi
...
check_feature_marker_coverage() {
  ...
  while IFS="$RECORD_SEPARATOR" read -r id source_id name upstream bedrock status docs notes; do
    ...
    if ! rg -q '<!--[[:space:]]*upstream-docs:' "$docs_file"; then
      broadcastclient "Feature $id ($name) is marked $status but ${docs_file#$ROOT_PATH/} has no <!-- upstream-docs: --> marker." >&2
      failed=1
    fi
  done < <(list_feature_records)
  return "$failed"
...
check_feature_symbol_present() {
  ...
  while IFS="$RECORD_SEPARATOR" read -r id source_id name upstream bedrock status docs notes; do
    ...
    if [ ! -d "$ROOT_PATH/packages/$pkg" ] && [ ! -d "$ROOT_PATH/services/$pkg" ]; then
      broadcastclient "Feature $id references unknown package directory: $bedrock" >&2
      failed=1
      continue
    fi
    ...
    if [ "$pkg" != "$bedrock" ]; then
      ...
      if ! rg -q --type go -w -F -- "$sym" "${search_paths[@]}" 2>/dev/null; then
        broadcastclient "Feature $id symbol not found in Go code: $bedrock" >&2
        failed=1
      fi
    fi
  done < <(list_feature_records)
...
drift() {
  ...
  broadcastclient "# Upstream Drift Report"
  printf '\n_Generated: %s_\n\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
