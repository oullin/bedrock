...
func (a app) precognitionHandler(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		a.container.Render(w, r, "Features/Forms/Precognition", protocol.Props{})
	case http.MethodPost:
		if err := protocol.ParseForm(r); err != nil {
			slog.Error("parse form", "error", err)
		...
		if len(errors) > 0 {
			...
			a.container.Render(w, r.WithContext(ctx), "Features/Forms/Precognition", protocol.Props{})
...
func (a app) wayfinderHandler(w http.ResponseWriter, r *http.Request) {
	a.container.Render(w, r, "Features/Forms/Wayfinder", protocol.Props{})
}
